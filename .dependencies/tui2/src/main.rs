...
fn main() -> io::Result<()> {
    ...
    let ghostex_mode = raw_args.iter().any(|arg| arg == "--ghostex");
    let ghostex_cleaned_args = raw_args
        .iter()
        .filter(|arg| arg.as_str() != "--ghostex")
        .cloned()
        .collect::<Vec<_>>();
    let args = match session::configure_from_args(&ghostex_cleaned_args) {
        Ok(args) => args,
        Err(err) => {
            eprintln!("error: {err}");
