...
impl AppState {
    pub(super) fn workspace_list_rect(&self) -> Rect {
        ...
        if self.ghostex_mode || self.sidebar_collapsed || sidebar.width <= 1 || sidebar.height == 0
        {
            return Rect::default();
        }
    ...
    pub(super) fn agent_panel_rect(&self) -> Rect {
        ...
        if self.ghostex_mode {
            return crate::ui::ghostex_sidebar_rect(sidebar);
        }
        let (_, detail_area) =
    ...
    pub(super) fn on_sidebar_section_divider(&self, col: u16, row: u16) -> bool {
        if self.ghostex_mode || self.sidebar_collapsed {
            return false;
        }
    ...
    pub(super) fn on_agent_panel_scope_toggle(&self, col: u16, row: u16) -> bool {
        if self.sidebar_collapsed || self.ghostex_mode {
            return false;
        }
    ...
    pub(super) fn ghostex_session_key_at_agent_panel_row(&self, row: u16) -> Option<String> {
        if !self.ghostex_mode || self.sidebar_collapsed {
            return None;
        }

        match crate::ui::ghostex_sidebar_row_at_screen_row(self, self.agent_panel_rect(), row) {
            Some(crate::ghostex::SidebarRow::Session(session)) => {
                Some(crate::ghostex::session_identity_key(&session))
            }
            _ => None,
        }
    }

    pub(super) fn ghostex_create_target_at_agent_panel_row(
        &self,
        row: u16,
        ...
        match crate::ui::ghostex_sidebar_row_at_screen_row(self, self.agent_panel_rect(), row) {
            Some(crate::ghostex::SidebarRow::NewTerminal {
                project_id,
                group_id,
            }) => Some((project_id, group_id)),
            _ => None,
        }
    }
}
