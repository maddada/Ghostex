                ...
                if in_sidebar {
                    ...
                    if on_new_button {
                        if self.ghostex_mode {
                            self.request_ghostex_create_terminal = true;
                        } else {
                    ...
                    if let Some((project_id, group_id)) =
                        self.ghostex_create_target_at_agent_panel_row(mouse.row)
                    {
                        self.request_ghostex_create_terminal_target = Some((project_id, group_id));
                        self.mode = Mode::Terminal;
                        return None;
                    }

                    if let Some(key) = self.ghostex_session_key_at_agent_panel_row(mouse.row) {
                        self.request_ghostex_attach_session_key = Some(key);
                        self.mode = Mode::Terminal;
                        return None;
                    }
