...
export function ModalGallerySection() {
  ...
  return (
    ...
    >
      <Note>
        Force-opens bypass the single app-modal slot; the real app can only ever have one window.
      </Note>
          ...
          <div className="cp-gallery-grid">
            {group.entries.map((galleryEntry) => (
              ...
              >
                ...
                {galleryEntry.note === undefined ? null : (
                  <span className="cp-gallery-note">{galleryEntry.note}</span>
                )}
              </button>
            ))}
          </div>
