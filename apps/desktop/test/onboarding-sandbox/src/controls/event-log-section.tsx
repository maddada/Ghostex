...
function EventRow({
  ...
  return (
    <div
      className={`cp-event cp-event--${event.kind}${expanded ? " is-expanded" : ""}`}
      data-kind={event.kind}
    >
      <button
        className="cp-event-head"
        onClick={onToggle}
        title={expandable ? "Show detail" : undefined}
        type="button"
      >
...
export function EventLogSection() {
  ...
  const visible = useMemo(
    () => events.filter((event) => !hiddenKinds.includes(event.kind)),
    [events, hiddenKinds],
  );
  ...
  const toggleKind = (kind: SimEventKind) => {
    setHiddenKinds(
      hiddenKinds.includes(kind) ? hiddenKinds.filter((k) => k !== kind) : [...hiddenKinds, kind],
    );
  };
    ...
    <div className={collapsed ? "cp-log is-collapsed" : "cp-log"}>
      ...
      {collapsed ? null : (
        <>
          ...
          >
            {visible.length === 0 ? (
              <p className="cp-log-empty">No events yet — launch the app.</p>
            ) : null}
            {visible.map((event) => {
              ...
              return (
                <div key={event.id}>
                  {showLaunchDivider ? (
                    <div className="cp-log-divider">launch #{event.launchIndex}</div>
                  ) : null}
                  <EventRow
                    event={event}
