...
export function createEngineActions(
  ...
  function refreshTipsNotices(): void {
    const state = get();
    const notices = tipsRuntimeProbed
      ? probedTipsNotices(state.env)
      : settingsDerivedTipsNotices(state.env);
    set(() => ({
      tipsNotices: notices,
      tipsBadgeCount: tipsBadgeCount(TITLEBAR_TIP_IDS.length, notices),
    }));
  ...
  function runTrackAGxserverBootstrap(): void {
    ...
    emit(
      "flow",
      `Track A: gxserver health probe resolved → ${scenario}`,
      undefined,
      CODE_REFS.healthProbe,
    );
    if (scenario === "healthyToolsAvailable") {
      emit(
        "flow",
        "replay_sidebar_gxserver_bootstrap — sidebar loads projects",
        undefined,
        CODE_REFS.bootstrap,
      );
  ...
  function runFirstRunOnboarding(source: string): void {
    ...
    const needsFirstLaunchSetup =
      next.firstLaunchSetupSeenRevision !== FIRST_LAUNCH_SETUP_SEEN_REVISION ||
      !next.firstLaunchSetupComplete;
    if (changed) {
  ...
  return {
    ...
    applyPreset: (presetId) => {
      ...
      updateEnv((current) =>
        typeof preset.apply.env === "function"
          ? preset.apply.env(current)
          : { ...current, ...preset.apply.env },
      );
