...
const FAKE_PROJECT_NAMES = ['ghostex', 'acme-api', 'design-system', 'infra-scripts', 'playground', 'docs-site'];

const FAKE_SESSION_TITLES = ['Implement onboarding sandbox', 'Review release notes', 'Fix sidebar diff stats'];
...
function fakeProjects(count: number): { id: string; name: string; sessions: string[] }[] {
  return Array.from({ length: Math.min(count, 12) }, (_, index) => ({
    ...
    name:
      FAKE_PROJECT_NAMES[index % FAKE_PROJECT_NAMES.length] + (index >= FAKE_PROJECT_NAMES.length ? `-${index}` : ''),
    ...
    <div className='sbx-window' data-focused={focused}>
      <div className='sbx-window-titlebar'>
        ...
        <div className='sbx-window-title'>
          ...
          {projects.length > 0 ? <span className='sbx-window-title-project'>— {projects[0].name}</span> : null}
        </div>
              ...
              <button
                ...
                onClick={() =>
                  emitEvent({
                    ...
                    detail: "Sandbox desktop only reports the click; recovering gxserver is the engine's job.",
                    kind: 'message',
        ...
        <div className='sbx-main'>
          ...
          <div className='sbx-terminal'>
            ...
            <div className='sbx-terminal-hint'>
              Simulated terminal at rest. The sandbox renders app chrome and the real onboarding modals; it does not run
              a shell.
            </div>
          </div>
        </div>
