...
const cleanShotImagePath =
  '/Users/madda/Library/Application Support/CleanShot/media/media_x/2026-05-23_Ghostex_13-53-53@2x.png';
...
describe('project board priority labels', () => {
  test('uses urgency words while preserving numeric bd values', () => {
    expect([0, 1, 2, 3].map((priority) => priorityLabel(priority))).toEqual(['Urgent', 'High', 'Medium', 'Low']);
  });
...
describe('project board filters', () => {
  ...
  test('filters by normalized priority and estimate without changing lane status', () => {
    expect(filterBoardTickets(tickets, '', '3', 'all', 'all').map((ticket) => ticket.id)).toEqual(['legacy-low']);
    expect(filterBoardTickets(tickets, '', 'all', 'none', 'all').map((ticket) => ticket.id)).toEqual(['medium-none']);
    expect(filterBoardTickets(tickets, '', '0', 'XS', 'all').map((ticket) => ticket.id)).toEqual(['urgent-xs']);
  });
  ...
  test('filters by tag alongside the other toolbar selections', () => {
    ...
    expect(filterBoardTickets(tickets, '', '0', 'all', 'docs').map((ticket) => ticket.id)).toEqual(['urgent-xs']);
...
describe('project board view preferences', () => {
  ...
  test('restores stored priority, estimate, sort, and tag selections', () => {
    ...
    expect(normalizeProjectBoardViewPreferences({ estimateFilter: 'none' }).estimateFilter).toBe('none');
  });
  ...
  test('keeps a stored tag the current board may no longer offer', () => {
    ...
    expect(normalizeProjectBoardViewPreferences({ tagFilter: 'retired-lane-tag' }).tagFilter).toBe('retired-lane-tag');
  ...
  test('keeps every offered toolbar option restorable', () => {
    for (const option of BOARD_SORT_OPTIONS) {
      expect(normalizeProjectBoardViewPreferences({ sortOption: option.value }).sortOption).toBe(option.value);
    }
  });
});
...
describe('project board assigned agent resolution', () => {
  ...
  test('matches a tool-suffixed assignee against the bare agent id', () => {
    ...
    expect(resolveAssignedAgentId('Gemini-CLI', [{ agentId: 'gemini', label: 'Gemini' }])).toBe('gemini');
  });
