...
export function ManageMarkdownBlockRenderer({
  ...
  switch (block.type) {
    ...
    case 'paragraph':
    default:
      return <p data-block-id={block.id}>{renderManageInlineMarkdown(block.content, annotations)}</p>;
  }
}
...
export function parseManageMarkdownLinkToken(
  text: string,
  index: number
): { nextIndex: number; node: ReactNode } | undefined {
...
export function parseManageMarkdownImageToken(
  text: string,
  index: number
): { nextIndex: number; node: ReactNode } | undefined {
