...
describe("Project Board form event handling", () => {
  ...
  test("keeps generated Kanban titles background-only after deterministic draft creation", () => {
    ...
    const createTicketSource = sourceBetweenIn(projectBoardAppSource, "const createTicket = async", "const deleteTicket = async");
    const titleGenerationSource = sourceBetweenIn(
      projectBoardAppSource,
      "const generateCreatedTicketTitle = async",
      "if (!createdIssue?.id)",
  ...
  test("closes edit-ticket Save before background persistence finishes", () => {
     ...
     */
    const saveSource = sourceBetweenIn(projectBoardAppSource, "const persistTicketDetail = async", "const createTicket = async");
    const toastSource = sourceBetweenIn(projectBoardAppSource, "const showProjectBoardToast = useCallback", "const setLocalTicketStatus");
  ...
  test("starts ticket work with the agent the bead is assigned to", () => {
     ...
     */
    const projectBoardSource = sourceFrom(projectBoardAppSource, "function ProjectBoardApp()");
    const startWorkSource = sourceBetweenIn(projectBoardAppSource, "const startTicketWork = async", "const selectTicketAgent =");
