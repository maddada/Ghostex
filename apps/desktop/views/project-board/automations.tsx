...
function AutomationGroupCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="divide-y divide-border/60 rounded-xl border border-border/80 bg-white/[0.03]">
      {children}
    </div>
  );
}
...
function AutomationDetailRow({
  ...
  return (
    <div className="flex min-h-11 items-center justify-between gap-4 px-4 py-2.5">
      ...
      <dd className="flex min-w-0 items-center gap-1.5 text-sm font-normal text-muted-foreground">
        {children}
      </dd>
    </div>
  );
...
export function AutomationComingSoonOverlay({ surfaceName }: { surfaceName: string }) {
  return (
    <section
      ...
      className="flex min-h-0 flex-1 items-center justify-center p-7"
    >
          ...
          >
            <span
              ...
              className={`size-1.5 shrink-0 rounded-full ${
                automation.enabled ? "bg-emerald-400/80" : "bg-white/20"
              }`}
            />
            <div className="min-w-0 flex-1">
              <div className="flex min-w-0 items-center gap-2">
                ...
                {unreadCount > 0 ? (
                  <span className="shrink-0 text-xs text-[var(--ghostex-accent)]">
                    {unreadCount} unread
                  </span>
                ) : null}
              </div>
        ...
        <div className="flex flex-col gap-2.5">
          ...
          <AutomationGroupCard>
            <dl className="divide-y divide-border/60">
              ...
              <AutomationDetailRow label="Mode">
                {describeAutomationMode(automation.executionMode)}
              </AutomationDetailRow>
              ...
              {automation.executionMode.kind === "thread" ? (
                <AutomationDetailRow label="Thread">
                  <span className="truncate">{automation.executionMode.sessionId}</span>
                </AutomationDetailRow>
              ) : null}
