import { type BeadsBridgeRequest, type BeadsBridgeResponse } from '../project-board-shared';
import {
...
export function sendBeadsRequest(request: Omit<BeadsBridgeRequest, 'requestId'>): Promise<BeadsBridgeResponse> {
  return new Promise((resolve, reject) => {
    ...
    const projectBeadsBridge = (window as ProjectBeadsWebKitWindow).webkit?.messageHandlers?.ghostexProjectBeads;
    if (projectBeadsBridge) {
      projectBeadsBridge.postMessage(message);
      return;
    }
    if (request.action === 'listIssues' && request.cwd) {
      void fetch(`file://${request.cwd}/.beads/issues.jsonl`)
        .then(() => reject(new Error('Beads bridge is unavailable outside Ghostex.')))
        .catch(() => {
