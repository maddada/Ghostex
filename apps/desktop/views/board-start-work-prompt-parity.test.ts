...
describe('board start-work prompt parity', () => {
  test('the Rust work-prompt template matches buildAgentWorkPrompt line for line', () => {
    const rustSource = readFileSync(new URL('../../../server/src/board_start_work.rs', import.meta.url), 'utf8');
    const functionSource = rustSource.slice(rustSource.indexOf('pub(crate) fn build_board_bead_work_prompt'));
