...
extern void GhostexGpuiMenuBarStatusSessionClicked(const char* project_id, const char* session_id);
...
@property(nonatomic, copy) NSString* title;
@property(nonatomic, copy) NSArray<GhostexGpuiMenuBarStatusSessionModel*>* sessions;
@end
...
static NSFont* GhostexGpuiMenuBarStatusFont(void) {
  return [NSFont monospacedDigitSystemFontOfSize:15.0 weight:NSFontWeightSemibold];
}

static NSColor* GhostexGpuiMenuBarStatusAvailableTextColor(void) {
  return [NSColor colorWithName:@"GhostexGpuiMenuBarAvailableText"
                dynamicProvider:^NSColor*(NSAppearance* appearance) {
                  ...
                  if ([bestMatch isEqualToString:NSAppearanceNameDarkAqua] ||
                      [bestMatch isEqualToString:NSAppearanceNameAccessibilityHighContrastDarkAqua]) {
...
static NSColor* GhostexGpuiMenuBarStatusTextColor(GhostexGpuiMenuBarStatusKind kind) {
  switch (kind) {
...
static NSDictionary<NSAttributedStringKey, id>* GhostexGpuiMenuBarStatusTextAttributes(
  GhostexGpuiMenuBarStatusKind kind
) {
  return @{
...
static NSSize GhostexGpuiMenuBarStatusTextSize(GhostexGpuiMenuBarStatusEntry entry) {
  NSAttributedString* label =
    [[NSAttributedString alloc] initWithString:GhostexGpuiMenuBarStatusCountLabel(entry.count)
                                    attributes:@{NSFontAttributeName: GhostexGpuiMenuBarStatusFont()}];
  return label.size;
}

static NSSize GhostexGpuiMenuBarStatusBadgeSize(GhostexGpuiMenuBarStatusEntry entry) {
...
static NSSize GhostexGpuiMenuBarStatusPreferredSize(
  const GhostexGpuiMenuBarStatusEntry* entries,
  NSUInteger count
) {
  CGFloat width = 2.0;
  CGFloat height = 22.0;
...
static void GhostexGpuiMenuBarStatusDrawEntries(
  ...
  for (NSUInteger index = 0; index < count; index += 1) {
    ...
    NSRect badgeRect = NSMakeRect(
      x,
      floor(NSMidY(bounds) - badgeSize.height / 2.0),
      badgeSize.width,
      badgeSize.height);

    [NSColor.controlBackgroundColor setFill];
    [[NSBezierPath bezierPathWithRoundedRect:badgeRect xRadius:6.0 yRadius:6.0] fill];
...
static NSImage* GhostexGpuiMenuBarStatusImage(
  const GhostexGpuiMenuBarStatusEntry* entries,
  NSUInteger count
) {
  ...
  [image lockFocus];
  GhostexGpuiMenuBarStatusDrawEntries(entries, count, NSMakeRect(0.0, 0.0, size.width, size.height));
  [image unlockFocus];
  image.template = NO;
  return image;
}
...
static void GhostexGpuiMenuBarStatusDrawPaddedTitle(
  NSString* title,
  NSFont* font,
  NSColor* textColor,
  ...
  NSRect textRect = NSMakeRect(
    ...
    MAX(0.0, NSWidth(bounds) - horizontalPadding * 2.0),
    textHeight);
...
static NSString* GhostexGpuiMenuBarStatusRelativeTime(NSString* timestamp) {
  ...
  NSDate* date = [fractionalFormatter dateFromString:timestamp] ?: [secondFormatter dateFromString:timestamp];
  if (date == nil) {
    return @"";
  }
...
static NSArray<GhostexGpuiMenuBarStatusProjectModel*>* GhostexGpuiCopyMenuBarStatusProjects(
  ...
  NSMutableArray<GhostexGpuiMenuBarStatusProjectModel*>* copiedProjects = [NSMutableArray array];
  uintptr_t safeProjectCount = MIN(projectCount, (uintptr_t)32);
  for (uintptr_t projectIndex = 0; projectIndex < safeProjectCount; projectIndex += 1) {
    ...
    project.projectId = GhostexGpuiMenuBarStatusStringFromCString(projectEntry.project_id);
    project.title = GhostexGpuiMenuBarStatusStringFromCString(projectEntry.title);

    NSMutableArray<GhostexGpuiMenuBarStatusSessionModel*>* sessions = [NSMutableArray array];
    if (projectEntry.sessions != NULL) {
      uintptr_t safeSessionCount = MIN(projectEntry.session_count, (uintptr_t)16);
      for (uintptr_t sessionIndex = 0; sessionIndex < safeSessionCount; sessionIndex += 1) {
        GhostexGpuiMenuBarStatusSessionEntry sessionEntry = projectEntry.sessions[sessionIndex];
        GhostexGpuiMenuBarStatusSessionModel* session =
          [[GhostexGpuiMenuBarStatusSessionModel alloc] init];
        session.sessionId = GhostexGpuiMenuBarStatusStringFromCString(sessionEntry.session_id);
        session.title = GhostexGpuiMenuBarStatusStringFromCString(sessionEntry.title);
        session.lastActiveAt = GhostexGpuiMenuBarStatusStringFromCString(sessionEntry.last_active_at);
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  CGFloat y = NSMaxY(self.bounds) - knobHeight - maxOffset * _knobOffsetFraction;
  [[NSColor.tertiaryLabelColor colorWithAlphaComponent:0.8] setFill];
  [[NSBezierPath bezierPathWithRoundedRect:NSMakeRect(0.0, y, NSWidth(self.bounds), knobHeight)
...
- (instancetype)initWithFrame:(NSRect)frameRect {
  ...
  if (self) {
    self.wantsLayer = YES;
    self.layer.backgroundColor = [NSColor colorWithCalibratedWhite:0x16 / 255.0 alpha:1.0].CGColor;
    self.layer.borderColor = [NSColor colorWithCalibratedWhite:0x3a / 255.0 alpha:0.72].CGColor;
...
- (void)drawRect:(NSRect)dirtyRect {
  [super drawRect:dirtyRect];
  GhostexGpuiMenuBarStatusDrawPaddedTitle(self.title, self.displayFont, self.textColor, 10.0, self.bounds);
}
@end
...
@property(nonatomic, copy) void (^hoverChanged)(GhostexGpuiMenuBarStatusSessionRow* row);
- (instancetype)initWithProjectId:(NSString*)projectId
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  NSRect iconRect = NSMakeRect(
    iconX,
    floor(NSMidY(self.bounds) - iconSize / 2.0),
    iconSize,
    iconSize);
  ...
  NSDictionary<NSAttributedStringKey, id>* titleAttributes = @{
    NSFontAttributeName: [NSFont systemFontOfSize:15.55 weight:NSFontWeightLight],
    NSForegroundColorAttributeName: [NSColor colorWithCalibratedRed:0xb4 / 255.0
                                                              green:0xb8 / 255.0
                                                               blue:0xc0 / 255.0
                                                              alpha:1.0],
  ...
  NSDictionary<NSAttributedStringKey, id>* timeAttributes = @{
    NSFontAttributeName: [NSFont monospacedDigitSystemFontOfSize:13.55 weight:NSFontWeightLight],
    NSForegroundColorAttributeName: [NSColor colorWithCalibratedWhite:0x4f / 255.0 alpha:1.0],
  };
  CGFloat titleX = NSMinX(self.bounds) + 41.0;
  CGFloat trailingWidth = 82.0;
  NSRect titleRect = NSMakeRect(
    titleX,
    floor(NSMidY(self.bounds) - 10.0),
    MAX(0.0, NSWidth(self.bounds) - titleX - trailingWidth - 14.0),
    20.0);
  [_title drawInRect:titleRect withAttributes:titleAttributes];

  if (_status == GhostexGpuiMenuBarStatusKindWorking) {
    NSRect squareRect = NSMakeRect(NSMaxX(self.bounds) - 21.0, floor(NSMidY(self.bounds) - 4.0), 8.0, 8.0);
    ...
    [_trailingText drawAtPoint:NSMakePoint(NSMaxX(self.bounds) - 13.0 - timeSize.width,
...
@property(nonatomic, copy) NSArray<GhostexGpuiMenuBarStatusProjectModel*>* projects;
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _panel = [[GhostexGpuiMenuBarStatusPanel alloc]
      initWithContentRect:NSMakeRect(0.0, 0.0, GhostexGpuiMenuBarStatusPanelWidth, GhostexGpuiMenuBarStatusMinPanelHeight)
                styleMask:NSWindowStyleMaskBorderless | NSWindowStyleMaskNonactivatingPanel
                  backing:NSBackingStoreBuffered
                    defer:NO];
    /*
     CDXC:GPUIMenuBarStatusItem 2026-06-26-06:05:
     The GPUI menu-bar primary click opens a native Running Agents dropdown, not a CEF or GPUI overlay. Keep it as a borderless non-activating AppKit panel with normal visible row controls, click-away dismissal, project/session callbacks, and Restart/Quit footer rows so opening it does not raise the main app.

     CDXC:GPUIMenuBarStatusItem 2026-06-26-06:29:
     Pixel parity with the native menu-bar dropdown requires a hover-only 2px scrollbar pinned to the panel's right edge. Keep it as visible AppKit chrome with the system scroller disabled; it must not introduce hidden hit regions or input rerouting.
     */
...
- (void)setProjects:(NSArray<GhostexGpuiMenuBarStatusProjectModel*>*)projects {
  _projects = [projects copy] ?: @[];
  [self rebuildRows];
}

- (CGFloat)contentWidth {
  return GhostexGpuiMenuBarStatusPanelWidth - GhostexGpuiMenuBarStatusContentHorizontalPadding * 2.0;
}

- (CGFloat)projectCardInnerWidth {
  return self.contentWidth - GhostexGpuiMenuBarStatusProjectCardHorizontalPadding * 2.0;
}
...
- (void)configureContent {
  ...
  contentView.layer.backgroundColor = [NSColor colorWithCalibratedWhite:0x1e / 255.0 alpha:1.0].CGColor;
  contentView.layer.borderColor = [NSColor colorWithCalibratedWhite:0x4f / 255.0 alpha:0.72].CGColor;
  contentView.layer.borderWidth = 1.0;
  contentView.layer.cornerRadius = 18.0;
  contentView.layer.masksToBounds = YES;
  _panel.contentView = contentView;

  _focusSink = [[GhostexGpuiMenuBarStatusFocusSink alloc] initWithFrame:NSMakeRect(-8.0, -8.0, 1.0, 1.0)];
  ...
  _rowsContentView = [[GhostexGpuiMenuBarStatusFlippedView alloc]
    initWithFrame:NSMakeRect(0.0, 0.0, self.contentWidth, GhostexGpuiMenuBarStatusEmptyHeight)];
  ...
  _scrollbarView = [[GhostexGpuiMenuBarStatusThinScrollbar alloc] initWithFrame:NSZeroRect];
  ...
  [footerStack addArrangedSubview:[self actionButtonWithTitle:@"Restart Ghostex"
                                                       action:@selector(restartGhostex:)]];
  [footerStack addArrangedSubview:[self actionButtonWithTitle:@"Quit Ghostex"
  ...
  _rowsHeightConstraint = [_rowsContainerView.heightAnchor constraintEqualToConstant:GhostexGpuiMenuBarStatusEmptyHeight];
  [NSLayoutConstraint activateConstraints:@[
    [rootStack.leadingAnchor constraintEqualToAnchor:contentView.leadingAnchor constant:GhostexGpuiMenuBarStatusContentHorizontalPadding],
    [rootStack.trailingAnchor constraintEqualToAnchor:contentView.trailingAnchor constant:-GhostexGpuiMenuBarStatusContentHorizontalPadding],
    [rootStack.topAnchor constraintEqualToAnchor:contentView.topAnchor constant:GhostexGpuiMenuBarStatusContentVerticalPadding],
    [rootStack.bottomAnchor constraintEqualToAnchor:contentView.bottomAnchor constant:-GhostexGpuiMenuBarStatusContentVerticalPadding],

    [_focusSink.leadingAnchor constraintEqualToAnchor:contentView.leadingAnchor constant:-8.0],
    [_focusSink.topAnchor constraintEqualToAnchor:contentView.topAnchor constant:-8.0],
    [_focusSink.widthAnchor constraintEqualToConstant:1.0],
    [_focusSink.heightAnchor constraintEqualToConstant:1.0],

    [_rowsContainerView.widthAnchor constraintEqualToConstant:self.contentWidth],
    _rowsHeightConstraint,
    [_scrollView.leadingAnchor constraintEqualToAnchor:_rowsContainerView.leadingAnchor],
    [_scrollView.trailingAnchor constraintEqualToAnchor:_rowsContainerView.trailingAnchor],
    [_scrollView.topAnchor constraintEqualToAnchor:_rowsContainerView.topAnchor],
    [_scrollView.bottomAnchor constraintEqualToAnchor:_rowsContainerView.bottomAnchor],
    [_scrollbarView.trailingAnchor constraintEqualToAnchor:contentView.trailingAnchor],
    [_scrollbarView.topAnchor constraintEqualToAnchor:_rowsContainerView.topAnchor],
    [_scrollbarView.bottomAnchor constraintEqualToAnchor:_rowsContainerView.bottomAnchor],
    [_scrollbarView.widthAnchor constraintEqualToConstant:GhostexGpuiMenuBarStatusScrollbarWidth],

    [_rowsStack.leadingAnchor constraintEqualToAnchor:_rowsContentView.leadingAnchor],
    [_rowsStack.trailingAnchor constraintEqualToAnchor:_rowsContentView.trailingAnchor],
    ...
    [footerStack.heightAnchor constraintEqualToConstant:GhostexGpuiMenuBarStatusFooterHeight],
  ]];
}

- (GhostexGpuiMenuBarStatusActionButton*)actionButtonWithTitle:(NSString*)title
                                                        action:(SEL)action {
  ...
  [button.heightAnchor constraintEqualToConstant:GhostexGpuiMenuBarStatusFooterRowHeight].active = YES;
...
- (NSArray<GhostexGpuiMenuBarStatusProjectModel*>*)nonEmptyProjects {
  NSMutableArray<GhostexGpuiMenuBarStatusProjectModel*>* nonEmptyProjects = [NSMutableArray array];
...
- (void)rebuildRows {
  ...
  NSArray<GhostexGpuiMenuBarStatusProjectModel*>* projects = [self nonEmptyProjects];
  if (projects.count == 0) {
  ...
  [_rowsContentView setFrameSize:NSMakeSize(self.contentWidth, [self preferredRowsHeight])];
...
- (NSTextField*)emptyLabel {
  ...
  [label.heightAnchor constraintEqualToConstant:GhostexGpuiMenuBarStatusEmptyHeight].active = YES;
  [label.widthAnchor constraintEqualToConstant:self.contentWidth].active = YES;
  return label;
}

- (NSStackView*)projectSection:(GhostexGpuiMenuBarStatusProjectModel*)project {
  ...
  [sectionStack.widthAnchor constraintEqualToConstant:self.contentWidth].active = YES;
  [sectionStack.heightAnchor constraintEqualToConstant:[self projectSectionHeight:project]].active = YES;
  return sectionStack;
}

- (NSView*)projectCard:(GhostexGpuiMenuBarStatusProjectModel*)project {
  GhostexGpuiMenuBarStatusProjectCardView* card =
    [[GhostexGpuiMenuBarStatusProjectCardView alloc] initWithFrame:NSZeroRect];
  ...
  for (GhostexGpuiMenuBarStatusSessionModel* session in sortedSessions) {
    [cardStack addArrangedSubview:[self sessionRowWithProject:project session:session]];
  }

  [NSLayoutConstraint activateConstraints:@[
    [card.widthAnchor constraintEqualToConstant:self.contentWidth],
    [card.heightAnchor constraintEqualToConstant:[self projectSessionsCardHeight:project]],
    [cardStack.leadingAnchor constraintEqualToAnchor:card.leadingAnchor constant:GhostexGpuiMenuBarStatusProjectCardHorizontalPadding],
    [cardStack.trailingAnchor constraintEqualToAnchor:card.trailingAnchor constant:-GhostexGpuiMenuBarStatusProjectCardHorizontalPadding],
    [cardStack.topAnchor constraintEqualToAnchor:card.topAnchor constant:GhostexGpuiMenuBarStatusProjectCardVerticalPadding],
    [cardStack.bottomAnchor constraintEqualToAnchor:card.bottomAnchor constant:-GhostexGpuiMenuBarStatusProjectCardVerticalPadding],
  ]];
  return card;
}

- (GhostexGpuiMenuBarStatusProjectButton*)projectButton:(GhostexGpuiMenuBarStatusProjectModel*)project {
  GhostexGpuiMenuBarStatusProjectButton* button =
    [[GhostexGpuiMenuBarStatusProjectButton alloc] initWithProjectId:project.projectId];
  button.title = project.title;
  button.target = self;
  button.action = @selector(projectClicked:);
  button.translatesAutoresizingMaskIntoConstraints = NO;
  [button.heightAnchor constraintEqualToConstant:GhostexGpuiMenuBarStatusProjectHeaderHeight].active = YES;
  [button.widthAnchor constraintEqualToConstant:self.contentWidth].active = YES;
  return button;
}

- (GhostexGpuiMenuBarStatusSessionRow*)sessionRowWithProject:(GhostexGpuiMenuBarStatusProjectModel*)project
                                                     session:(GhostexGpuiMenuBarStatusSessionModel*)session {
  GhostexGpuiMenuBarStatusSessionRow* row =
    [[GhostexGpuiMenuBarStatusSessionRow alloc] initWithProjectId:project.projectId
                                                          session:session];
  row.target = self;
  ...
  [row.heightAnchor constraintEqualToConstant:GhostexGpuiMenuBarStatusRowHeight].active = YES;
  [row.widthAnchor constraintEqualToConstant:self.projectCardInnerWidth].active = YES;
  return row;
}
...
- (CGFloat)preferredRowsHeight {
  ...
  height += MAX((NSInteger)projects.count - 1, 0) * GhostexGpuiMenuBarStatusProjectSectionSpacing;
  return height;
}

- (CGFloat)projectSectionHeight:(GhostexGpuiMenuBarStatusProjectModel*)project {
...
- (CGFloat)projectSessionsCardHeight:(GhostexGpuiMenuBarStatusProjectModel*)project {
...
- (CGFloat)preferredPanelHeight {
  CGFloat requestedHeight = [self preferredRowsHeight] +
                            GhostexGpuiMenuBarStatusFooterHeight +
...
- (void)showFromStatusButton:(NSStatusBarButton*)button {
  ...
  _rowsHeightConstraint.constant = panelHeight -
                                   GhostexGpuiMenuBarStatusFooterHeight -
                                   GhostexGpuiMenuBarStatusContentVerticalPadding * 2.0;
  [_panel.contentView layoutSubtreeIfNeeded];
  [_panel setFrame:[self panelFrameWithSize:NSMakeSize(GhostexGpuiMenuBarStatusPanelWidth, panelHeight)
                                  anchoredTo:button]
           display:YES];
...
- (NSRect)panelFrameWithSize:(NSSize)size anchoredTo:(NSStatusBarButton*)button {
  NSScreen* fallbackScreen = NSScreen.mainScreen ?: NSScreen.screens.firstObject;
  NSRect buttonFrame = button.window
    ? [button.window convertRectToScreen:[button convertRect:button.bounds toView:nil]]
    : NSMakeRect(NSEvent.mouseLocation.x, NSEvent.mouseLocation.y, 0.0, 0.0);
  NSRect screenFrame = fallbackScreen.visibleFrame;
  CGFloat x = MIN(MAX(NSMinX(screenFrame) + 8.0, NSMidX(buttonFrame) - size.width / 2.0),
                  NSMaxX(screenFrame) - size.width - 8.0);
  CGFloat proposedY = NSMinY(buttonFrame) - size.height - 8.0;
  CGFloat y = proposedY >= NSMinY(screenFrame) + 8.0 ? proposedY : NSMaxY(buttonFrame) + 8.0;
...
- (void)updateScrollbar {
  ...
  if (!_isMouseInsidePanel || visibleHeight <= 0.0 || contentHeight <= visibleHeight + 1.0) {
    _scrollbarView.hidden = YES;
    return;
  }
  ...
  _scrollbarView.knobOffsetFraction = MIN(1.0, MAX(0.0, _scrollView.contentView.bounds.origin.y / maxOffset));
}
...
- (void)sessionClicked:(GhostexGpuiMenuBarStatusSessionRow*)sender {
  [self dismissPanel];
  GhostexGpuiMenuBarStatusActivateApplication();
  GhostexGpuiMenuBarStatusSessionClicked(sender.projectId.UTF8String, sender.sessionId.UTF8String);
}
...
@property(nonatomic, strong) GhostexGpuiMenuBarStatusPanelController* panelController;
- (void)statusItemClicked:(NSStatusBarButton*)sender;
- (void)updateProjects:(NSArray<GhostexGpuiMenuBarStatusProjectModel*>*)projects;
- (void)dismissPanel;
@end
...
- (void)statusItemClicked:(NSStatusBarButton*)sender {
  ...
  /*
   CDXC:GPUIMenuBarStatusItem 2026-06-26-06:29:
   The status button is already registered for leftMouseUp, but AppKit may dispatch this action after NSApp.currentEvent is nil or has moved to another event. Match the Swift host by ignoring only explicit secondary-click events and Control-clicks.
   */
  if (event != nil &&
...
static GhostexGpuiMenuBarStatusTarget* GhostexGpuiMenuBarStatusClickTarget = nil;

static GhostexGpuiMenuBarStatusTarget* GhostexGpuiEnsureMenuBarStatusTarget(void) {
  if (GhostexGpuiMenuBarStatusClickTarget == nil) {
    GhostexGpuiMenuBarStatusClickTarget = [[GhostexGpuiMenuBarStatusTarget alloc] init];
  }
  return GhostexGpuiMenuBarStatusClickTarget;
}

static NSUInteger GhostexGpuiMenuBarStatusEntries(
  ...
  if (count == 0) {
    /*
     ...
     When Rust applies instead of hides, the menu-bar item must remain visible as the Running Agents dropdown target. Render an available-style zero chip for the all-empty state; only the Rust hide path removes the NSStatusItem.
     */
    entries[count++] = (GhostexGpuiMenuBarStatusEntry){
...
static void GhostexGpuiApplyMenuBarStatusItemOnMain(
  ...
  /*
   ...
   The compact NSStatusItem badge is still driven by primitive counts selected by Rust; renderer-owned menu-bar payloads remain rejected.

   CDXC:GPUIMenuBarStatusItem 2026-06-26-06:05:
   The Running Agents dropdown may receive only Rust-owned sanitized project/session ids, titles, status, order, and timestamps from the strict parser. AppKit copies that data at the FFI boundary and owns only presentation plus fixed callbacks, never paths, URLs, command text, stdout/stderr, tokens, environment, terminal content, renderer JSON, hidden hit regions, or overlay routing.
  */
  GhostexGpuiMenuBarStatusTarget* target = GhostexGpuiEnsureMenuBarStatusTarget();
  [target updateProjects:copiedProjects ?: @[]];

  GhostexGpuiMenuBarStatusEntry entries[3] = {0};
  NSUInteger count =
    GhostexGpuiMenuBarStatusEntries(attentionCount, workingCount, availableCount, entries);
  if (count == 0) {
    /*
     CDXC:GPUIMenuBarStatusItem 2026-06-26-05:44:
     Applying primitive counts is a visible-state operation; hiding is reserved for the explicit Rust None path. Keep a defensive available-style zero entry here so future edits to the entry builder cannot make an all-zero apply remove the dropdown target.
     */
    entries[count++] = (GhostexGpuiMenuBarStatusEntry){
      .kind = GhostexGpuiMenuBarStatusKindAvailable,
      .count = 0,
...
void GhostexGpuiApplyMenuBarStatusItemWithProjects(
  ...
  if (NSThread.isMainThread) {
    GhostexGpuiApplyMenuBarStatusItemOnMain(
      attentionCount,
      workingCount,
      availableCount,
      copiedProjects);
...
void GhostexGpuiApplyMenuBarStatusItem(
  uint64_t attentionCount,
  uint64_t workingCount,
  uint64_t availableCount
) {
