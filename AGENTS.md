...
3. **Rust cores**: `packages/gx-core` (sidebar), `packages/gx-chat-core` (the only chat brain: desktop, web and, through UniFFI, the phone), `packages/gx-protocol` (wire types).
...
Chat rules go only into `gx-chat-core`; the TypeScript chat brain and the React chat were deleted on 2026-09-25. Start no new work in the React Kanban/Automate pages (bug fixes only). The desktop runs no JavaScript outside its CEF pages: the QuickJS app runtime was deleted on 2026-09-25, so never add a JavaScript engine or service back to it. When a fix lands higher than a layer that could own it, say why in your report. Full guide with examples: `ai/where-to-fix.md`.
...
│   │   ├── src/       # Rust
│   │   ├── sidebar/   # CEF page entries (find, kanban, manage)
...
│   ├── core-ui/       # the shared React UI (find, settings, modals, Markdown, assets)
...
- **The window chrome is native Rust; there is no titlebar page** (the React `titlebar-host.html` that last drew the Tips and Resources panels was deleted on 2026-09-26). There is no titlebar row, and the work area header (project breadcrumb, Start/Open/Commit, the `⋯` menu, the panel toggles) is drawn by `apps/desktop/src/app/render/workarea_header/`; which views are open is the view panel's own tab strip, `apps/desktop/src/app/render/view_tab_strip.rs`, with its menus in `app/view_tab_menus.rs` and its model in `app/view_panel.rs`. Menus, popups, tips and resources behaviour live in `apps/desktop/src/app/titlebar/`, and the Tips and Resources panels are drawn by `apps/desktop/src/app/window/titlebar_panels/`; the view list comes from `titlebar_mode_switcher_items` in `apps/desktop/src/app/helpers/titlebar.rs` (thin wrappers in `app/workarea.rs` and `app/model/runtime_state.rs`). This work belongs in those Rust files.
...
Shared gxserver contracts live in `packages/shared/` (for example `gxserver-protocol.ts`); the desktop and the GPUI web build talk to gxserver from Rust (`apps/desktop/src/app/gx_store/`, `apps/gpui-web/src/app/gx_store/`, both through `gx_rpc` in `gx_store/rpc.rs`). The React web app was deleted on 2026-09-24; `packages/core-ui/` is the React UI still used by the desktop's CEF pages and the mobile Find view, and the React sidebar that lived there (`sidebar-app.tsx` and its cards, sections and stories) was deleted with it; find it in git history when a native port cites it. The desktop sidebar is native Rust (`apps/desktop/src/app/native_sidebar/` for the renderer, `apps/desktop/src/app/gx_store/` for the store that feeds it), the TypeScript sidebar page and its `index.html` entry were deleted on 2026-09-21, and the desktop's QuickJS app runtime (`apps/desktop/sidebar/service/`, `gxserver-runtime/`, `native-quick-access/`, the `rquickjs` engine and its bundle) was deleted on 2026-09-25 after its last behaviour moved to gxserver, `gx-core` and `gx_store/` (`docs/2026-09-25/app-runtime-port/`); the chat's socket to gxserver is Rust too (`packages/gx-chat-client`). Client storage (`client-storage.sqlite3`) starts and is read in Rust (`packages/client-storage-native`, crate `ghostex-client-storage`, the Rust side of `packages/client-storage/`).
...
- Mobile uses the Rust chat core: native React Native views (`apps/mobile/app/src/chat/`) fed by `gx-chat-core` through UniFFI (`packages/gx-chat-mobile`; user decisions 2026-09-21 and 2026-09-25, native only). What is left in `packages/core-ui/chat/` is not a chat: it is the Markdown renderer the CEF pages use, the Lexical prompt input of the GhostexEditor helper, and the draft storage behind the Stashed Prompts modal.
- **The GPUI chat and the React Native chat must match** (user decision 2026-09-26): unless the user says otherwise, every chat fix, feature or visual change lands in both in the same change. Put the rule in `gx-chat-core` so both get it at once; when the fix is in drawing or input, change `apps/desktop/src/app/native_chat/` and `apps/mobile/app/src/chat/` together. If one platform cannot do it (an RN `TextInput` cannot replace the text it shows, for example), say so in your report instead of leaving the two apart silently.
...
- When a chat message did not send (Enter did nothing, the message came back, a send error), read `ai/chat-send-debugging.md` first: gxserver logs every refused send with the terminal screen it saw.
- Keep shared storage ownership, validation, budgets, revision checks, and recovery rules in `packages/client-storage/`. On the desktop the file is read and written by the Rust storage doors (`packages/client-storage-native`, `gx_store/sidebar_ui_storage.rs`, `gx_store/records_storage.rs`), which enforce the catalog's bounds; they must preserve existing saved data when migrating from browser storage.
...
When asked to mock up a UI or a flow, build static HTML under `docs/<YYYY-MM-DD>/<topic>/`, never Storybook stories or product code: one `.html` per screen or state plus an `index.html` hub in flow order, shared `shared.css` / `shared.js`, short design notes above or below the device frame (never beside it), and a frame that keeps its real width and scrolls sideways on a narrow window instead of squashing. Feedback arrives as CSS selector paths, so every landmark the user might point at (frames, cards, rows, steps, buttons, sheets, notes) gets a descriptive class or data attribute on the element they would click, plus a selector cheat sheet in `index.html`. Copy says "computer", never "Mac", and uses user-facing feature names ("Easy Connect", not "Tailcat"). Match the Kanban / Automate look (near-black page, `#161616` panels, `#1d1d1d` cards, hairline borders) unless told otherwise, and screenshot with headless Chrome to fix clipping and overflow before reporting. Full conventions: `ai/ux-mockups.md`.
...
- Routine disk logs must belong to one of the **Diagnostic logs** areas on the Debugging page (`DIAGNOSTIC_LOGGING_SCENARIOS` in `packages/shared/ghostex-settings/diagnostic-logging.ts`) and may write only while both **Show debug UI controls** and that unexpired area are enabled. Reuse the closest existing area; add a new one only when none fits, and delete an area when its last writer goes. Do not add unscoped routine disk logging; errors, crashes, and important warnings remain unconditional.
