...
- Keep shared storage ownership, validation, budgets, revision checks, and recovery rules in `packages/client-storage/`. On the desktop the file is read and written by the Rust storage doors (`packages/chat-runtime`, `gx_store/sidebar_ui_storage.rs`, `gx_store/records_storage.rs`), which enforce the catalog's bounds; they must preserve existing saved data when migrating from browser storage.
