  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('close', (status) => {
      if (status !== 0) {
        reject(
          new Error(
            `${command} ${args.join(' ')} failed (${status})${stderr.length ? `\n${Buffer.concat(stderr).toString('utf8').trim()}` : ''}`
          )
        );
        return;
      }
...
export function getRelease(version, { required = true } = {}) {
  assertVersion(version);
  let response = run('gh', ['api', `repos/${RELEASE_REPO}/releases/tags/v${version}`], {
    allowFailure: true,
    capture: true,
  });
  if (response.status !== 0) {
    const releases = run('gh', ['api', `repos/${RELEASE_REPO}/releases?per_page=100`], {
      allowFailure: true,
      capture: true,
    });
...
export function createInitialState({
  ...
  assertVersion(version);
  assertSha(sourceSha);
  if (!new Set(['stable', 'prerelease', 'test']).has(channel))
    throw new Error(`Unsupported release channel: ${channel}`);
  if (channel !== 'stable' && updateSparkle)
    throw new Error(`${channel} releases cannot update the production Sparkle feed`);
...
export function ensureDraftRelease({
  channel = 'stable',
  packages = null,
  sourceSha,
  updateSparkle = true,
  ...
  if (packages && JSON.stringify(releasePackageNames(version, state)) !== JSON.stringify(packages)) {
    throw new Error(
      `Release package scope ${JSON.stringify(releasePackageNames(version, state))} does not match ${JSON.stringify(packages)}`
    );
  }
...
export function validateStateIdentity(state, { sourceSha, version }) {
  ...
  if (JSON.stringify(state.expected) !== JSON.stringify(exactExpected)) {
    throw new Error(
      `Release expected allowlist is invalid: ${JSON.stringify(state.expected)} != ${JSON.stringify(exactExpected)}`
    );
  }
  if (state.source_compatibility !== undefined) {
    if (
      !state.source_compatibility ||
      typeof state.source_compatibility !== 'object' ||
      Array.isArray(state.source_compatibility)
    ) {
...
function sourcePathAffectsPackage(file, packageName) {
  if (
    ...
    file === 'tooling/release-assemble-resumable.mjs' ||
    file === 'tooling/release-resumable.mjs' ||
    file === 'tooling/release-state-lib.mjs'
  ) {
    return false;
  }
...
export function rebaseDraftSource(version, { newSourceSha, rebuildPackages }) {
  ...
  const packageVersion = JSON.parse(
    run('git', ['show', `${newSourceSha}:package.json`], { capture: true }).stdout
  ).version;
  if (packageVersion !== version)
    throw new Error(`package.json at ${newSourceSha} is ${packageVersion}; expected ${version}`);

  const changedPaths = run('git', ['diff', '--name-only', oldSourceSha, newSourceSha], { capture: true })
    .stdout.split(/\r?\n/)
    .filter(Boolean);
    ...
    if (requestedRebuilds.has(packageName)) {
      ...
      for (const assetName of contract.assets) {
        if (findAsset(release, assetName) || findAsset(release, `${assetName}${METADATA_SUFFIX}`)) {
          throw new Error(
            `Refusing to rebase while rebuild package ${packageName} already has staged asset ${assetName}`
          );
        }
      }
      continue;
  ...
  for (const packageName of contracts.keys()) {
    ...
    sourceCompatibility[packageName] = {
      audited_changes: [...new Set([...(priorCompatibility?.audited_changes ?? []), ...changedPaths])],
  ...
  for (const assetName of contract.assets) {
    ...
    if (existingMetadataAsset) {
      ...
      if (
        ...
        existing.architecture !== contract.architecture ||
    ...
    prepared.push(
      (async () => {
        ...
        if (downloadedSha !== expectedSha) {
          throw new Error(
            `Downloaded ${assetName} checksum ${downloadedSha} does not match v${source.state.version} digest ${expectedSha}`
          );
        }
      })()
    );
...
async function stageReusablePackageAssets({
  artifactDirectory,
  contract,
  packageName,
  reusedFrom,
...
export async function reuseGxserverPackagesFromRelease(version, { fromVersion }) {
  ...
  try {
    ...
    for (const item of prepared) {
      ...
      console.log(
        `${targetContracts.get(item.packageName).label}: reused byte-for-byte from public v${fromVersion} with verified checksums`
      );
    }
    return staged;
  } finally {
  ...
  try {
    for (const assetName of contract.assets) {
      ...
      if (downloadedSha !== publishedSha) {
        throw new Error(
          `Downloaded ${assetName} checksum ${downloadedSha} does not match v${fromVersion} digest ${publishedSha}`
        );
      }
    }
    const reusedFrom = {
...
export function validateStagedRelease(version, { requireComplete = false, sourceSha = null } = {}) {
  ...
  for (const [packageName, contract] of contracts) {
    ...
    for (const name of contract.assets) {
      ...
      if (
        ...
        metadata.architecture !== contract.architecture ||
        metadata.asset !== name ||
        metadata.sha256 !== actualSha ||
        Number(metadata.size) !== actualSize
      ) {
    ...
    if (entries.length === contract.assets.length) {
      ...
      const recordedAssetsMatch =
        recorded?.assets && entries.every((entry) => recorded.assets[entry.asset] === entry.sha256);
  ...
  if (requireComplete && errors.length > 0)
    throw new Error(`Release staging is incomplete or invalid:\n- ${errors.join('\n- ')}`);
...
export function printStatus(version) {
  ...
  for (const [packageName, contract] of contracts) {
    ...
    if (ready) {
      ...
      lines.push(
        `${contract.label.padEnd(26)} failed — resume notarization ${result.state.macos_notarization.submission_id}`
      );
    } else if (packageName === 'macos-arm64' && result.state.macos_notarization?.signed_dmg_run_id) {
      lines.push(
        `${contract.label.padEnd(26)} signed — submit preserved run ${result.state.macos_notarization.signed_dmg_run_id}`
      );
    } else {
      const reason =
        result.errors
  ...
  lines.push(
    `${'GitHub release'.padEnd(26)} ${result.release.draft ? (complete ? 'ready to assemble' : 'waiting for packages') : 'published'}`
  );
  lines.push(
    `${'Sparkle'.padEnd(26)} ${result.state.sparkle?.published ? 'published' : result.state.sparkle?.requested ? 'not published' : 'disabled'}`
  );
...
export function planAdvanceAfterStaging(version, result, stagedPackage) {
  ...
  if (!result.completed[stagedPackage])
    throw new Error(`${stagedPackage} is not validly staged and cannot advance the release`);
  const decisions = [];
  for (const [packageName, contract] of contracts) {
  ...
  const assemblyNeeded =
    complete &&
    (result.release.draft ||
      !result.state.github_release?.published ||
...
export function recordMacosSigned(
  version,
  { channel, dmgSha256, signedDmgRunId, sourceSha, updateSparkle, workflowSha }
) {
  ensureDraftRelease({ channel, sourceSha, updateSparkle, version, workflowSha });
  const release = getRelease(version);
  const state = readJsonAsset(release, STATE_ASSET);
...
export function replaceStagedAsset(version, { assetName, expectedOldSha }) {
  ...
  if (actual !== expectedOldSha)
    throw new Error(`Expected old SHA256 ${expectedOldSha}, but ${assetName} is ${actual}`);
  ...
  const packageName = [...selectedReleaseContracts(version, state)].find(([, contract]) =>
    contract.assets.includes(assetName)
  )?.[0];
