...
async function validateBundledCodeServerRuntime({ arch, resourcesRoot, expectedNodePtyPrebuild }) {
  ...
  const codeServerVscodeRipgrep = path.join(
    codeServerRoot,
    'lib',
    'vscode',
    'node_modules',
...
function findFirstPortlessNodeRuntimePath(portlessRoot) {
  ...
  if (result.status !== 0) {
    const output = [result.stderr, result.stdout]
      ...
      .join('\n');
    throw new MacosAppBundleValidationError(
      `Portless duplicate Node runtime scan failed.${output ? `\n${output}` : ''}`
    );
  }
...
async function validateOnDemandResourceShape({ arch, onDemandManifest, resourcesRoot, sharedBd }) {
  ...
  for (const assetKey of requiredAssetKeys) {
    const asset = onDemandManifest.assets?.[assetKey];
    if (!asset?.name || !/^[0-9a-f]{64}$/.test(asset?.sha256 ?? '')) {
      throw new MacosAppBundleValidationError(`${arch} on-demand manifest is missing a valid entry for ${assetKey}.`);
    }
  }
...
async function validateUnbundledCefComponent({ appPath, arch, appName, onDemandManifest }) {
  const bundledFramework = path.join(appPath, 'Contents', 'Frameworks', 'Chromium Embedded Framework.framework');
  ...
  const helperApps = (await readdir(path.join(appPath, 'Contents', 'Frameworks'), { withFileTypes: true })).filter(
    (entry) => entry.isDirectory() && helperPattern.test(entry.name)
  );
  ...
  if (!asset || !/^[0-9a-f]{64}$/.test(asset.sha256 ?? '')) {
    throw new MacosAppBundleValidationError(`${arch} app does not seal a valid CEF component checksum.`);
  }
}
...
async function validateUnbundledCodeServerComponent({ arch, onDemandManifest, resourcesRoot }) {
  ...
  const asset = component?.platforms?.['darwin-arm64'];
  if (!asset || !/^[0-9a-f]{64}$/.test(asset.sha256 ?? '')) {
    throw new MacosAppBundleValidationError(`${arch} app does not seal a valid code-server component checksum.`);
  }
}
...
async function assertMachOContainsArch(binaryPath, arch) {
  ...
  const archs = runFile('/usr/bin/lipo', ['-archs', binaryPath], { label: `lipo -archs ${binaryPath}` })
    .stdout.trim()
    .split(/\s+/)
    .filter(Boolean);
  if (!archs.includes(arch)) {
    throw new MacosAppBundleValidationError(
      `${binaryPath} does not contain required ${arch} slice. Found: ${archs.join(', ') || 'none'}.`
    );
  }
}
