...
switch (command) {
  case 'start': {
    ...
    const packages = onlyMacos ? ['gxserver-linux-x64', 'gxserver-linux-arm64', 'macos-arm64'] : null;
  ...
  case 'retry': {
    ...
    if (target === 'macos-submit') {
      ...
      console.log(
        `macOS notarization: submit preserved signed run ${notarization.signed_dmg_run_id}; no compilation will run`
      );
  ...
  case 'verify': {
    ...
    const tagLines = run('git', ['ls-remote', 'origin', `refs/tags/v${version}`, `refs/tags/v${version}^{}`], {
      capture: true,
    }).stdout.split(/\r?\n/);
    ...
    if ((peeled ?? direct)?.split(/\s+/)[0] !== state.source_sha)
      throw new Error(`v${version} does not resolve to ${state.source_sha}`);
    if (state.sparkle.requested && !state.sparkle.published)
      throw new Error(`Sparkle has not been published for ${version}`);
    console.log(
      `v${version} is public with the exact verified asset allowlist${state.sparkle.requested ? ' and Sparkle is published' : ''}.`
    );
    break;
  }
  case 'replace': {
    ...
    assertVersion(version);
    if (!assetName || !/^[0-9a-f]{64}$/.test(expectedOldSha ?? ''))
      throw new Error('An asset and full --expect-sha are required');
