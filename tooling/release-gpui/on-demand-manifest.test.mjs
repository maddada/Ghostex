...
describe('on-demand manifest v2', () => {
  ...
  test('requires the exact code-server outer checksum sidecar name in the sealed manifest', () => {
    ...
    releaseManifest.components['code-server'].platforms['windows-x64'].sha256SidecarName = 'wrong.tar.gz.sha256';
  ...
  test.each([
    ...
    expect(() =>
      authenticateComponentChecksumSidecar(contents, 'code-server-version-windows-x64.tar.gz', digest)
...
describe('component-tag publisher idempotency', () => {
  ...
  test('plans create-if-missing, no-op-if-matching, and error-if-sha-mismatch', async () => {
    ...
    expect(
      planComponentRelease({
        ...
        release: {
          exists: true,
          assets: [{ name: assets[0].assetName, size: assets[0].sizeBytes, digest: `sha256:${assets[0].sha256}` }],
        },
      })
    ...
    expect(() =>
      planComponentRelease({
        ...
        release: {
          exists: true,
          assets: [{ name: assets[0].assetName, size: assets[0].sizeBytes, digest: `sha256:${'b'.repeat(64)}` }],
        },
      })
  ...
  test('publishes exact filename-bound checksum sidecars with component archives', async () => {
    ...
    expect(sidecars).toMatchObject([{ assetName: `${archiveName}.sha256`, filePath: `${archivePath}.sha256` }]);
    expect(
