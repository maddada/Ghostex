...
describe('product provenance records', () => {
  ...
  test('rejects malformed digests, sizes, and artifact names', () => {
    ...
    expect(() => androidRecord({ artifacts: [{ name: 'ghostex-android.apk', sha256: 'short', size: 1 }] })).toThrow(
      /sha256/u
    );
    expect(() => androidRecord({ artifacts: [{ name: 'ghostex-android.apk', sha256: digestA, size: -1 }] })).toThrow(
      /size/u
    );
    expect(() => androidRecord({ artifacts: [{ name: '../escape.apk', sha256: digestA, size: 1 }] })).toThrow(
      /plain file name/u
    );
...
describe('release provenance asset', () => {
  test('names and validates the published record', () => {
    ...
    expect(() => validateReleaseProvenance({ ...record, products: { 'macos-arm64': androidRecord() } })).toThrow(
      /product must equal macos-arm64/u
    );
  });
      ...
      baselines: [
        ...
        { publishedAt: '2026-07-01T00:00:00.000Z', repo: TRUSTED_REPO, tag: 'v7.7.0' },
        {
          ...
          provenance: buildReleaseProvenance({
            plan: {},
            products: {
              android: androidRecord({ originTag: 'v7.5.0', productVersion: '7.5.0', releaseVersion: '7.5.0' }),
            },
...
describe('reuse verification', () => {
  ...
  test('rejects a draft release and a non-ancestor origin commit', () => {
    ...
    expect(accept({ evidence: { isAncestor: () => false } }).failures.join(' ')).toMatch(
      /is not an ancestor of the source commit/u
    );
  });
  ...
  test('rejects an incompatible scope: a version-stamped product from another release', () => {
    const result = verifyReuseCandidate({
      candidate: releaseCandidate(
        macosRecord({ originTag: 'v7.7.0', productVersion: '7.7.0', releaseVersion: '7.7.0' })
      ),
      evidence: alwaysAncestor,
      fingerprint: digestA,
