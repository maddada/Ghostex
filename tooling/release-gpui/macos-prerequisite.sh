...
runtime)
	...
	[[ -f "$CODE_SERVER_LINUX_X64_ARCHIVE" ]] || {
		echo "macOS runtime preparation requires Linux x64 code-server archive: $CODE_SERVER_LINUX_X64_ARCHIVE" >&2
		exit 1
	}
	[[ -f "$CODE_SERVER_LINUX_ARM64_ARCHIVE" ]] || {
		echo "macOS runtime preparation requires Linux arm64 code-server archive: $CODE_SERVER_LINUX_ARM64_ARCHIVE" >&2
		exit 1
	}
...
rust)
	...
	for binary_path in apps/desktop/target/release/ghostex-gpui apps/desktop/target/release/ghostex-gpui-cef-helper; do
		[[ -x "$REPO_ROOT/$binary_path" ]] || {
			echo "Rust build is missing $binary_path" >&2
			exit 1
		}
	...
	[[ -f "$CEF_VERSION_HEADER" ]] || {
		echo "Rust build did not produce $CEF_VERSION_HEADER" >&2
		exit 1
	}
