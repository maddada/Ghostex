...
[[ -f "$DMG" ]] || {
	echo "Signed DMG is missing: $DMG" >&2
	exit 1
}
...
submit)
	OUTPUT="${4:-}"
	[[ -n "$OUTPUT" ]] || {
		echo "Submission output path is required" >&2
		exit 2
	}
...
poll)
	SUBMISSION_ID="${4:-}"
	[[ -n "$SUBMISSION_ID" ]] || {
		echo "Existing submission ID is required" >&2
		exit 2
	}
	...
	[[ "${STATUS:-}" == "Accepted" ]] || {
		echo "Apple notarization is still in progress; retry only the poll/staple stage" >&2
		exit 1
	}
