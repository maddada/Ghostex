 ...
 *
 * tooling/packaging/aur/ghostex-bin/PKGBUILD.template is the single source of truth.
...
const packageDirectory = path.join(repoRoot, 'tooling/packaging/aur/ghostex-bin');
 ...
 * duplicated under tooling/packaging/, so there is only ever one copy to keep current.
 */
...
function sha256FromGitHubMetadata(version, repo) {
  const result = spawnSync('gh', ['release', 'view', `v${version}`, '--repo', repo, '--json', 'assets,isDraft'], {
    encoding: 'utf8',
  });
  ...
  if (clone.status !== 0) {
    throw new Error(
      `Could not clone ${aurRemote}: ${(clone.stderr || clone.stdout || 'unknown error').trim()}\n` +
        ...
        'account that owns it. See tooling/packaging/aur/ghostex-bin/README.md for the one-time setup.'
    );
  }
