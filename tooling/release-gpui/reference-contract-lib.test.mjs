...
`;
    ...
    expect([...extractManagedTooltipPlacements(library)]).toEqual(['Auto', 'Left', 'BelowLeft']);
    expect([...extractManagedTooltipPlacementUsages(application)]).toEqual(['Left', 'Below']);

    const { missing } = missingManagedTooltipPlacements(library, [{ path: 'gpui/src/main.rs', source: application }]);
...
`;
    ...
    expect(
      missingRequiredRustMethods(library, ['items_padding_bottom', 'scrollbar_show', 'scrollbar_thickness']).missing
