...
import { BEADS_PACKAGE_ID, BEADS_SOURCE_REVISION_SHORT, BEADS_VERSION } from './beads-release.mjs';
...
export async function smokeTestPackagedBeads(binaryValue) {
  ...
  if (!expectedVersion.test(versionOutput)) {
    throw new Error(
      `Packaged Beads identity mismatch: expected ${BEADS_PACKAGE_ID}, ` + `got ${JSON.stringify(versionOutput)}`
    );
  }
  ...
  try {
    ...
    const initResult = await run(binaryPath, ['init', '--non-interactive', '--skip-agents', '--skip-hooks'], {
      cwd: repository,
      env,
    });
    ...
    if (!/Backend:\s*dolt/iu.test(initOutput) || !/Mode:\s*embedded/iu.test(initOutput)) {
      throw new Error(
        'Packaged Beads init did not report the required embedded Dolt backend. ' + `Output:\n${initOutput.trim()}`
      );
    }

    const metadataPath = path.join(repository, '.beads', 'metadata.json');
    const metadata = JSON.parse(await readFile(metadataPath, 'utf8'));
    if (metadata.backend !== 'dolt' || metadata.database !== 'dolt' || metadata.dolt_mode !== 'embedded') {
      throw new Error(`Packaged Beads workspace is not embedded Dolt: ${JSON.stringify(metadata)}`);
    }
    ...
    const issues = Array.isArray(listPayload)
      ...
      : Array.isArray(listPayload?.data)
        ? listPayload.data
        : listPayload?.issues;
    const createdIssue = Array.isArray(issues) ? issues.find((issue) => issue?.id === issueId) : undefined;
...
async function main() {
  ...
  if (!binaryPath) {
    throw new Error('Usage: node tooling/smoke-test-packaged-beads.mjs <packaged-bd-path>');
  }
