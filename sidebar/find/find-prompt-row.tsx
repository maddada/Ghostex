...
import {
  flattenPromptLineWithOffsets,
  splitHighlightedSegments,
} from "./find-prompt-highlight";
  ...
  return (
    <span className="ghostex-find-row-prompt">
      {segments.map((segment, position) =>
        segment.highlighted ? (
          <mark
            className="bg-transparent font-semibold text-[var(--ghostex-find-match)]"
            key={position}
          >
            {segment.text}
          </mark>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        selected
          ? "bg-accent/70 ring-1 ring-inset ring-border"
          : "hover:bg-accent/30",
      )}
    ...
    >
      <span className="flex w-3 shrink-0 justify-center pt-0.5">
        {row.favorite ? (
          <IconStarFilled aria-label="Favorite" className="size-3 text-amber-400" />
        ) : null}
      </span>
