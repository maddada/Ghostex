...
- **Enable transparency** `windowGlass` (one of auto | frosted | opaque; default auto): Let your desktop show through the window. Use transparency picks Automatic (dark mode only), Always or Never. Option labels: auto = Glass in dark mode, frosted = Always glass, opaque = Always opaque.
- **What shows behind the glass** `windowGlassSource` (one of desktopAndWindows | wallpaper | customImage | video; default desktopAndWindows): Desktop and windows, your wallpaper, a picture you choose, or a video playing behind the glass. Option labels: desktopAndWindows = Desktop and windows, wallpaper = Wallpaper only, customImage = Custom image, video = Video.
