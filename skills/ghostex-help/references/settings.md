...
- **What shows behind the glass** `windowGlassSource` (one of desktopAndWindows | wallpaper | customImage | video | live; default desktopAndWindows): Desktop and windows, your wallpaper, a picture you choose, a video, or a Live animated background in your theme colours. Option labels: desktopAndWindows = Desktop and windows, wallpaper = Wallpaper only, customImage = Custom image, video = Video, live = Live.
- **Picture position** `windowGlassImagePlacement` (one of static | desktop; default static): Where the wallpaper, custom picture or video sits behind the glass. Stays with the desktop can trail the window while you drag it. Option labels: static = Moves with the window, desktop = Stays with the desktop.
...
- **Play only when plugged in** `windowGlassVideoOnlyOnPower` (boolean, default true): Pause the glass video or Live background while your computer runs on battery. It always pauses when Ghostex is in the background.
- **Live background for dark mode** `windowGlassLiveStyleDark` (one of aurora | ink | drift | nebula | silk | bokeh | waves | mesh; default aurora): The animated background the glass shows in dark mode when Live shows behind the glass. Option labels: aurora = Aurora, ink = Ink, drift = Drift, nebula = Nebula, silk = Silk, bokeh = Bokeh, waves = Waves, mesh = Mesh.
- **Live background for light mode** `windowGlassLiveStyleLight` (one of aurora | ink | drift | nebula | silk | bokeh | waves | mesh; default drift): The animated background the glass shows in light mode when Live shows behind the glass. Option labels: aurora = Aurora, ink = Ink, drift = Drift, nebula = Nebula, silk = Silk, bokeh = Bokeh, waves = Waves, mesh = Mesh.
- **Live background speed** `windowGlassLiveSpeed` (number default 1): How fast the Live background moves, from a quarter of its pace to twice as fast.
- **Live background brightness** `windowGlassLiveBrightness` (number default 45): How bright the Live background glows behind the glass. Lower keeps it a subtle glow.
...
- **Show debug UI controls** `debuggingMode` (boolean, default false): Show diagnostic logs, storage statistics, and Copy Resume and Copy Attach in session menus.
