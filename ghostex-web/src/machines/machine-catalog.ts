...
async function initializeMachineCatalogOnce(): Promise<void> {
  ...
  try {
    ...
    const machines = [
      localMachine,
      ...state.machines.filter((machine) => machine.machineId !== "local"),
    ];
    updateState({
...
async function fetchPrimaryMachine(): Promise<GhostexWebMachine> {
  ...
  if (
    result.protocolVersion !== GXSERVER_PROTOCOL_VERSION
    || !result.authToken
    || !result.machineLabel
  ) {
...
function isBootstrapEnvelope(
  value: unknown,
): value is { result: GxserverWebBootstrapResult } {
  if (!isObject(value) || value.ok !== true || value.product !== GXSERVER_PRODUCT || !isObject(value.result)) {
    return false;
  }
  const result = value.result;
  return typeof result.authToken === "string"
