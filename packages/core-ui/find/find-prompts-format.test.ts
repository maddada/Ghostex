...
describe("find prompt formatting", () => {
  ...
  it("renders the full last-active line in UTC", () => {
    ...
    expect(formatLastActiveFull(1_755_000_000)).toMatch(
      /^last active [A-Z][a-z]{2} \d+ \d{2}:\d{2} UTC$/u,
    );
  });
  ...
  it("builds the usage tail only from present fields", () => {
    expect(
      formatPromptMetaLine({
        ...
        thinking: "",
        usage: {
          cacheRead: 0, cacheWrite: 0, contextWindow: 0, cost: 0,
          input: 0, output: 0, ratePercent: 0,
        },
      }),
    ).toBe("");
    ...
    expect(
      formatPromptMetaLine({
        ...
        thinking: "high",
        usage: {
          cacheRead: 3, cacheWrite: 0, contextWindow: 200000, cost: 1.5,
          input: 10, output: 20, ratePercent: 12.5,
        },
      }),
