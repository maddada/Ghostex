...
const VEIL_SCALE: readonly [number, number] = [0.75, 0.35];
const VEIL_OFFSET: readonly [number, number] = [0, -0.8];
const VEIL_DIM = 0.8;

export function compileShaderProgram(gl: WebGLRenderingContext, vertex: string, fragment: string): WebGLProgram | null {
...
export function releaseWebGl(gl: WebGLRenderingContext): void {
  gl.getExtension('WEBGL_lose_context')?.loseContext();
}

export type DarkVeilProps = {
  /** Owns the GL context and the animation loop only while true; false tears both down. */
  active: boolean;
  hueShift?: number;
...
export function DarkVeil({
  ...
  useEffect(() => {
    ...
    canvas.width = Math.max(1, Math.round(width * resolutionScale * pixelRatio));
    canvas.height = Math.max(1, Math.round(height * resolutionScale * pixelRatio));
    ...
    gl.uniform1f(uniform('uScanFreq'), scanlineFrequency);
    gl.uniform1f(uniform('uWarp'), warpAmount);
    const timeUniform = uniform('uTime');
    const startedAt = performance.now();
    const reduced = prefersReducedMotion();
    let frame = 0;
    const draw = () => {
      gl.uniform1f(timeUniform, ((performance.now() - startedAt) / 1000) * speed);
