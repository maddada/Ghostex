...
export const DEFAULT_SIDEBAR_PROJECT_COLLECTION_COLOR = SIDEBAR_PROJECT_COLLECTION_COLORS[0];
...
function sanitizeSidebarProjectCollections(
  ...
  for (const rawCollection of rawCollections) {
    ...
    const collectionId = typeof candidate.collectionId === 'string' ? candidate.collectionId.trim().slice(0, 120) : '';
    ...
    const color =
      typeof candidate.color === 'string' && /^#[0-9a-f]{6}$/iu.test(candidate.color)
        ...
        : candidate.color === 'transparent'
          ...
          : SIDEBAR_PROJECT_COLLECTION_COLORS[collections.length % SIDEBAR_PROJECT_COLLECTION_COLORS.length];
  ...
  return {
    ...
    nextCollectionNumber:
      typeof rawNextCollectionNumber === 'number' && Number.isSafeInteger(rawNextCollectionNumber)
  ...
  return {
    ...
    state: {
      collections: [
        ...
        {
          color:
            SIDEBAR_PROJECT_COLLECTION_COLORS[(nextCollectionNumber - 1) % SIDEBAR_PROJECT_COLLECTION_COLORS.length],
          collectionId,
...
export function moveProjectsToSidebarCollection(
  ...
  const uniqueProjectIds = [...new Set(projectIds.map((projectId) => projectId.trim()))].filter(Boolean);
  if (uniqueProjectIds.length === 0) {
    return state;
  }
  if (collectionId) {
    const targetCollection = state.collections.find((collection) => collection.collectionId === collectionId);
    if (!targetCollection) {
      return state;
    }
  }
  ...
  return {
    ...
    collections: state.collections
      .map((collection) => ({
        ...collection,
        projectIds: collection.projectIds.filter((candidate) => !movingProjectIds.has(candidate)),
      }))
      .map((collection) =>
...
export function reorderSidebarProjectCollections(
  ...
  const orderByProjectId = new Map(projectIdsInOrder.map((projectId, index) => [projectId, index]));
  return {
    ...state,
...
export function reorderSidebarProjectCollectionDefinitions(
  ...
  const collectionById = new Map(state.collections.map((collection) => [collection.collectionId, collection]));
