...
describe('getSidebarCommandRunFeedbackDuration', () => {
  test('should use the requested success and error display durations', () => {
    ...
    expect(getSidebarCommandRunFeedbackDuration('success')).toBe(SIDEBAR_COMMAND_SUCCESS_FEEDBACK_DURATION_MS);
    expect(getSidebarCommandRunFeedbackDuration('error')).toBe(SIDEBAR_COMMAND_ERROR_FEEDBACK_DURATION_MS);
  });
});
