...
import type { SidebarCommandRunState, SidebarCommandRunStateChangedMessage } from '../shared/session-grid-contract';
...
export function applySidebarCommandRunStateChangedMessage(
  ...
  if (current && current.status === nextStatus && haveSameRunIds(current.activeRunIds, nextActiveRunIds)) {
    return current;
  }
...
export function getSidebarCommandRunFeedbackDuration(status: SidebarCommandRunState): number | undefined {
  switch (status) {
    case 'success':
...
export function reconcileSidebarCommandRunFeedbackStates(
  ...
  const nextEntries = Object.entries(commandRunStates).filter(([commandId]) => validCommandIds.has(commandId));
