...
import { expectSettingsPatch, findSidebarV2Row, waitForSidebarV2 } from './sidebar-v2.story-helpers';
...
export const FiltersByProjectScope: Story = {
  play: async ({ canvasElement, step }) => {
    ...
    await step('open the scope menu with every project plus Quick', async () => {
      fireEvent.click(await findRequiredElement(root, '.sidebar-v2-scope-trigger', 'scope trigger'));
    ...
    await step("carry each project's own icon into the scope menu", async () => {
      ...
      expect(ghostexItem.querySelector('.sidebar-v2-project-icon[data-icon-variant="tabler"]')).toBeTruthy();
...
export const SearchIncludesPreviousSessions: Story = {
  play: async ({ canvasElement, step, userEvent }) => {
    ...
    await step('offer the matching closed sessions below the inbox', async () => {
      ...
      expect(previousGroup.getBoundingClientRect().top).toBeGreaterThanOrEqual(root.getBoundingClientRect().top);
    });
  },
};
...
export const RenamesFromTheRow: Story = {
  play: async ({ canvasElement, step }) => {
    ...
    await step('commit the rename with the same message the modal posts', async () => {
      const row = await findSidebarV2Row(storyRoot, 'v2-quick-idle');
      const input = await findRequiredElement(row, '.sidebar-v2-row-rename-input', 'rename input');
...
export const SettlesAndSnoozesFromHoverActions: Story = {
  play: async ({ canvasElement, step }) => {
    ...
    await step('settle an inbox card from its hover check', async () => {
      const row = await findSidebarV2Row(storyRoot, 'v2-quick-idle');
      fireEvent.click(await findRequiredElement(row, '[aria-label="Settle session"]', 'settle action'));
    ...
    await step('refuse a second click while the write is unanswered', async () => {
      const row = await findSidebarV2Row(storyRoot, 'v2-quick-idle');
      const settle = await findRequiredElement(row, '[aria-label="Settle session"]', 'settle action');
    ...
    await step('open the snooze presets from the clock button', async () => {
      ...
      fireEvent.click(await findRequiredElement(row, '[aria-label="Snooze session"]', 'snooze action'));
    ...
    await step('snooze with a wake time strictly in the future', async () => {
      ...
      await waitFor(() => {
        const message = getSidebarStoryMessages().find((entry) => entry.type === 'snoozeSession');
    ...
    await step('un-settle from the settled shelf', async () => {
      ...
      fireEvent.click(await findRequiredElement(row, '[aria-label="Un-settle session"]', 'un-settle action'));
    ...
    await step('wake early from the snoozed shelf', async () => {
      resetSidebarStoryMessages();
      fireEvent.click(
        await findRequiredElement(root, '.sidebar-v2-shelf-header[data-tone="snoozed"]', 'snoozed shelf header')
      );
      const row = await findSidebarV2Row(storyRoot, 'v2-ghostex-snoozed');
      fireEvent.click(await findRequiredElement(row, '[aria-label="Wake session now"]', 'wake action'));
...
export const RunsLifecycleContextMenuActions: Story = {
  play: async ({ canvasElement, step }) => {
    ...
    await step('expand the Snooze submenu instead of guessing a preset', async () => {
      ...
      fireEvent.click(preset);
      await waitFor(() => {
        expect(getSidebarStoryMessages().some((entry) => entry.type === 'snoozeSession')).toBe(true);
      });
    });
...
export const RunsSessionContextMenuActions: Story = {
  play: async ({ canvasElement, step }) => {
    ...
    await step('unpin a pinned session from its hover chip', async () => {
      resetSidebarStoryMessages();
      const row = await findSidebarV2Row(storyRoot, 'v2-ghostex-pinned');
      const unpin = await findRequiredElement(row, '[aria-label="Unpin session"]', 'unpin action');
...
export const RendersPullRequestStateOnCards: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('mark each badge with the review state that colors it', async () => {
      ...
      const draftBadge = await findRequiredElement(draftRow, '.sidebar-v2-row-pr', 'draft PR badge');
      expect(draftBadge.getAttribute('data-pr-state')).toBe('draft');

      const closedRow = await findSidebarV2Row(storyRoot, 'v2-zmx-done');
      const closedBadge = await findRequiredElement(closedRow, '.sidebar-v2-row-pr', 'closed PR badge');
      ...
      const mergedBadge = await findRequiredElement(mergedRow, '.sidebar-v2-row-pr', 'merged PR badge');
    ...
    await step('hover text names the branch and the review state', async () => {
      ...
      expect(git.getAttribute('title')).toBe('ghostex/sidebar-v2-inbox · #128 · Open · +412 −87');
    });
    ...
    await step('give rows without git data no third line at all', async () => {
      ...
      expect(idleRow.closest('.sidebar-v2-row-item')?.getAttribute('data-card-lines')).toBe('2');
    });
...
export const HeaderHasOneCreateControl: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('the shared header keeps Sort & Filter and nothing that creates', async () => {
      await findRequiredElement(header, '.reference-sidebar-section-sort-action', 'Sort & Filter trigger');
    ...
    await step("V2's toolbar owns the only create control", async () => {
      await findRequiredElement(root, '.sidebar-v2-toolbar .sidebar-v2-create-button', 'plain create button');
      await findRequiredElement(root, '.sidebar-v2-toolbar .sidebar-v2-create-chevron', 'create chevron');
    });
    ...
    await step('the plain + creates in the resolved project, never in Quick', async () => {
      ...
      expect(
        getSidebarStoryMessages().some(
          (message) =>
            message.type === 'runSidebarAgent' && (message as { groupId?: string }).groupId !== projectGroupId
        )
      ).toBe(false);
    });
...
export const ClassicSidebarKeepsItsCreateTrio: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('the Quick header still creates browsers, terminals, and agents', async () => {
      const header = await findRequiredElement(storyRoot, '[data-reference-section="quick"]', 'Quick section header');
      ...
      await findRequiredElement(header, '.reference-sidebar-section-agent-picker', 'agent split button');
      await findRequiredElement(header, '.reference-sidebar-section-sort-action', 'Sort & Filter trigger');
    });
...
export const BrowserShelfLeadsTheFlatList: Story = {
  play: async ({ canvasElement, step }) => {
    ...
    await step("the Browser shelf header is the flat list's first row", async () => {
      ...
      const browserRowIndex = items.findIndex((item) => item.querySelector('[data-session-id="v2-ghostex-browser"]'));
      const inboxCardIndex = items.findIndex((item) => item.querySelector('[data-session-id="v2-ghostex-pinned"]'));
    ...
    await step('the shelves keep browser, snoozed, settled order', async () => {
      const tones = [...root.querySelectorAll<HTMLElement>('.sidebar-v2-list .sidebar-v2-shelf-header')].map((shelf) =>
        shelf.getAttribute('data-tone')
      );
...
async function openSidebarV2RowMenu(storyRoot: ParentNode, sessionId: string): Promise<void> {
  ...
  await waitFor(() => {
    return expect(storyRoot.querySelector('.sidebar-v2-session-context-menu [role="menuitem"]')).toBeTruthy();
  });
}
...
export const ShowsV1ParityContextMenuItems: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('the items V2 deliberately did not adopt stay absent', async () => {
      ...
      for (const label of ['Remote Access', 'Move to New Group', 'Sleep below', 'Close below', 'Pop Out Pane']) {
          ...
          return expect(
            modalHost.opened.some(
              (message) =>
                ...
                (message as { message?: string }).message === 'Write the 6.9.0 release notes from the merged PRs'
            )
          ).toBe(true);
        });
...
async function openSidebarV2GroupMenu(storyRoot: ParentNode, groupId: string): Promise<void> {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("emit V1's project-header DOM for every grouped row", async () => {
      ...
      for (const section of sections) {
        ...
        expect(section.getAttribute('data-sidebar-group-id')).toBe(section.getAttribute('data-sidebar-v2-group-id'));
        ...
        expect(head!.querySelector('.group-collapse-button.section-titlebar-toggle')).toBeTruthy();
        expect(head!.querySelector('.group-title-handle > .group-title-button > .group-title')).toBeTruthy();
...
export const CollapsesGroupedProjectsThroughTheSharedState: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    const section = async () => findRequiredElement(root, '[data-sidebar-v2-group-id="v2-project-zmx"]', 'zmx group');
    ...
    await step("persist it through the sidebar's shared collapse state", async () => {
      await waitFor(() => {
        const stored = window.localStorage.getItem('ghostex-sidebar-ui-collapse-state:window:main');
...
export const ClosesEveryMemberCheckoutOfAGroupedProject: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('close a merged project through every one of its members', async () => {
      ...
      expect(readSidebarV2MenuLabels(storyRoot)).toEqual(['Group across machines', 'Close Project']);
      fireEvent.click(
        [...storyRoot.querySelectorAll<HTMLElement>('.sidebar-v2-session-context-menu [role="menuitem"]')].find(
          (item) => item.textContent?.trim() === 'Close Project'
        )!
      );
      ...
      expect(
        getSidebarStoryMessages().filter((message) => message.type === 'closeWorkspaceProjectForGroup').length
      ).toBe(3);
    });
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("sections and dividers are the menu grid's own children", async () => {
      ...
      for (const childClass of childClasses) {
        expect(
          childClass.includes('session-context-menu-section') || childClass.includes('session-context-menu-divider')
        ).toBe(true);
      }
    });
    ...
    await step('Tag as flies out into its own stacked panel', async () => {
      ...
      expect(Math.round(submenuBounds.left)).toBe(
        Math.round(Math.max(margin, Math.min(tagAsBounds.left, window.innerWidth - submenuBounds.width - margin)))
      );
      expect(Math.round(submenuBounds.top)).toBe(
        Math.round(
          Math.max(margin, Math.min(tagAsBounds.bottom + 4, window.innerHeight - submenuBounds.height - margin))
        )
      );
...
export const GroupMenuMatchesTheClassicContextMenuChrome: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('the grouping choice flies out as its own panel', async () => {
       ...
       */
      expect(chevron!.getBoundingClientRect().right).toBeLessThanOrEqual(menu.getBoundingClientRect().right);
...
export const ReordersGroupedProjectsByDrag: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    const groupHead = async (groupId: string) =>
      findRequiredElement(root, `[data-sidebar-v2-group-id="${groupId}"] .group-head`, `group head for ${groupId}`);

    await step('start from the merged repository above the non-git project', async () => {
      await waitFor(() => {
        return expect(
          readSidebarV2GroupSections(root).map((section) => section.getAttribute('data-sidebar-v2-group-id'))
    ...
    await step('drag the non-git project above the merged repository', async () => {
      ...
      expect(getSidebarStoryMessages().filter((message) => message.type === 'syncGroupOrder').length).toBe(1);
    });
    ...
    await step("wear V1's drag chrome while a project row is in flight", async () => {
      ...
      await waitFor(() => {
        return expect(
          [...root.querySelectorAll<HTMLElement>('[data-group-drop-position]')].map(
            (section) =>
              `${section.getAttribute('data-sidebar-v2-group-id')}:${section.getAttribute('data-group-drop-position')}`
          )
      ...
      expect(getSidebarStoryMessages().filter((message) => message.type === 'syncGroupOrder').length).toBe(1);
      await waitFor(() => {
        return expect(root.querySelector('[data-sidebar-v2-group-id][data-dragging="true"]')).toBeNull();
      });
    });
  },
};
