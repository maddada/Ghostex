...
export async function findSidebarV2Row(
 ...
 */
export async function expectSettingsPatch(
  key: string,
  value: unknown,
): Promise<void> {
  await waitFor(() => {
 ...
 */
export async function expectProjectGroupingOverridePatch(
  ...
  await waitFor(() => {
    const matched = getSidebarStoryMessages().some((message: SidebarToExtensionMessage) => {
      ...
      const overrides = (message.patch as Record<string, unknown> | undefined)?.[
