...
export type WorktreeDeleteModalProps = {
  ...
  onDelete: (projectId: string, options: { deleteLocalBranch: boolean; deleteRemoteBranch: boolean }) => void;
  theme?: SidebarTheme;
};
...
export function WorktreeDeleteModal({
  ...
  const remoteBranchLabel = draft.remoteBranchName ? `${remoteName}/${draft.remoteBranchName}` : `${remoteName} branch`;
  const selectedBranchDeletes = [
    deleteLocalBranch && draft.canDeleteLocalBranch && localBranchName ? `local branch ${localBranchName}` : '',
    ...
    >
      ...
      >
        ...
        <div className='worktree-delete-modal-body'>
          <p className='worktree-delete-modal-question'>Delete worktree &quot;{draft.worktreeName}&quot;?</p>
          {draft.hasChanges ? (
            <div className='worktree-delete-status-block'>
              <p className='worktree-delete-status-heading'>This worktree has uncommitted changes:</p>
          ...
          <p className='worktree-delete-modal-note'>
            ...
            {selectedBranchDeletes.length > 0 ? ` and delete ${selectedBranchDeletes.join(' and ')}.` : '.'}
          </p>
        </div>
        <DialogFooter className='worktree-delete-modal-actions'>
          <Button className='worktree-delete-modal-button' onClick={onCancel} type='button' variant='outline'>
            Cancel
          </Button>
          {draft.hasChanges ? (
            <Button
