...
import { IconAlertTriangle, IconPower, IconQrcode } from '@tabler/icons-react';
 ...
 */
export function EasyConnectCard({ remote, rpcAvailable }: { remote: RemoteAccessState; rpcAvailable: boolean }) {
    ...
    >
      <div className='settings-remote-path-head'>
        <span className='settings-remote-path-title'>
          ...
          <label htmlFor={toggleId}>Easy Connect (QR/Token)</label>
        </span>
        <Switch
      ...
      {isOn ? (
        <div className='settings-remote-qr-block settings-remote-easy-connect-qr-block'>
          {easyConnectCode ? (
            <QrCode alt='Easy Connect pairing code' className='settings-remote-qr' value={easyConnectCode.payload} />
          ) : (
          ...
          <div className='settings-remote-qr-meta'>
            ...
            {easyConnectCode ? (
              <span>
                Pairs as <strong>{easyConnectCode.code.user}</strong> on <strong>{easyConnectCode.code.name}</strong>.
                Nothing to type on the phone.
              </span>
            ) : null}
            <span>
              The code refreshes after each pairing. Any phone or computer that scans the current code can pair; remove
              a device below to unpair it.
            </span>
            {easyConnectCode ? (
        ...
        <div className='settings-remote-off-block'>
          ...
          <div className='settings-remote-off-text'>
            ...
            <span className='settings-management-detail'>
              Ghostex keeps it running while the app is open. You can turn it off any time; paired phones simply stop
