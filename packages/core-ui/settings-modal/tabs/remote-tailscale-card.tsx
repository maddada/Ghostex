 ...
 * User: the two pairing paths are expandable cards so only one QR code is in front of the user at a time.
 * Collapsed, this card is its header row (icon, title, badges, chevron); the checklist and its QR render only in the expanded body.
 *
 * CDXC:RemotePairing 2026-09-03 DECISION:
 ...
 * The header switch is the Easy Connect card's switch in the same spot: a sibling of the header button with stopPropagation, so toggling never expands the card.
 * It writes Settings.remoteTailscaleEnabled; nothing is sent to gxserver because Tailscale itself is not managed by Ghostex.
 * Off: the card is collapsed and cannot be expanded (header button disabled, no chevron), it is dimmed like a hidden machine tile, the status badge reads Off, and no QR is rendered because the body never mounts.
 */
export function TailscaleCard({
  ...
  const detectedBadge = !enabled
    ? { label: 'Off', tone: 'disabled' as const }
    : !tailscale
      ? undefined
      : tailscaleRunning
  ...
  return (
    ...
    >
      <div className='settings-remote-path-head'>
        ...
        <Switch
          aria-label='Turn the Tailscale path on or off'
          checked={enabled}
      ...
      {open ? (
        <div className='settings-remote-path-body' id={bodyId}>
          <p className='settings-management-description'>
            Your phone joins your tailnet and connects over SSH. Nothing to enable here; follow the checklist, then scan
            the code at the end with the app.
          </p>
