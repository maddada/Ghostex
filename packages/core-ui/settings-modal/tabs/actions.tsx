...
import { DEFAULT_SIDEBAR_COMMAND_ICON, type SidebarCommandIcon } from '../../../shared/sidebar-command-icons';
...
export function ActionsSettingsTab({
  ...
  */
  const hasConfiguredActions = useMemo(
    () => [...orderedGlobalCommands, ...orderedCommands].some((command) => isSidebarCommandConfigured(command)),
        ...
        <div className='settings-page-width flex flex-col gap-6 px-5 pb-5'>
          <SettingsSection title={editorScope === 'global' ? 'Global Action' : 'Action'}>
            <ActionSettingsEditor
              ...
              onDelete={editorCommandId ? () => deleteCommand(editorScope, editorCommandId) : undefined}
...
export function ActionSettingsEditor({
  ...
  const [icon, setIcon] = useState<SidebarCommandIcon>(draft.icon ?? DEFAULT_SIDEBAR_COMMAND_ICON);
  ...
  const [url, setUrl] = useState(
    draft.url ?? ((lockedActionType ?? draft.actionType) === 'browser' ? DEFAULT_BROWSER_ACTION_URL : '')
  );
  ...
  const getDraft = (): SettingsCommandDraft => ({
    ...
    links:
      actionType === 'terminal'
        ? links.map((link) => ({ target: link.target, url: link.url.trim() })).filter((link) => link.url.length > 0)
        : undefined,
    name: trimmedName,
  ...
  const updateLink = (index: number, update: Partial<SidebarCommandLink>) => {
    setLinks((currentLinks) =>
      currentLinks.map((link, linkIndex) => (linkIndex === index ? { ...link, ...update } : link))
    );
  };
...
export function SettingsActionIcon({ command }: { command: SidebarCommandButton }) {
  return <SidebarCommandIconGlyph icon={command.icon ?? DEFAULT_SIDEBAR_COMMAND_ICON} stroke={1.8} />;
}
