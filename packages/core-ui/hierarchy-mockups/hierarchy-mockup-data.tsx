      ...
      { name: "Personal", collapsed: true, runningCount: 1, projects: [] },
      {
        ...
        projects: [
          ...
          },
          {
            ...
            browserTabs: [],
            sessions: [
              { kind: "session", agent: "claude", title: "Deploy landing page", status: "none" },
            ],
          },
        ],
      },
...
export function TerminalIcon({ color = "#9aa3af" }: { color?: string }) {
  return (
    <svg className="hxm-icon" viewBox="0 0 16 16" aria-hidden>
      ...
      <path d="M 4.6 6.2 L 7 8.2 L 4.6 10.2" fill="none" stroke={color} strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
export function Chevron({ collapsed }: { collapsed?: boolean }) {
  return (
    <svg className={`hxm-chevron${collapsed ? " hxm-chevron-collapsed" : ""}`} viewBox="0 0 16 16" aria-hidden>
      <path d="M 5 3.5 L 11 8 L 5 12.5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
