...
describe('groupRecentProjectsByMachine', () => {
  test('keeps local and remote recency order within their machine sections', () => {
    ...
    expect(grouped.local.map((project) => project.projectId)).toEqual(['agent-manager-x', 'open-design']);
    expect(grouped.remoteByMachineId.get('main-machine')?.map((project) => project.projectId)).toEqual([
