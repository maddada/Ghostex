...
function AutomatePage({ initialTab = "automations" }: { initialTab?: SurfaceTab }) {
  ...
  const setEnabled = (automation: AutomationDefinition, enabled: boolean) => {
    setAutomations((current) =>
      current.map((candidate) =>
        candidate.id === automation.id ? { ...candidate, enabled } : candidate,
      ),
    );
  };
  const markRead = (run: AutomationRun) => {
    setRuns((current) =>
      current.map((candidate) =>
        candidate.id === run.id ? { ...candidate, isUnread: false } : candidate,
      ),
    );
  };
          ...
          <>
            ...
            <AutomationRunDetail
              ...
              automation={automations.find(
                (candidate) =>
                  candidate.id ===
                  visibleRuns.find((run) => run.id === selectedRunId)?.automationId,
              )}
              onArchive={noop}
              onMarkRead={markRead}
              onOpenSession={noop}
