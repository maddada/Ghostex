...
import { createInitialSidebarStoreDataState, resetSidebarStore, useSidebarStore } from './sidebar-store';
...
describe('sidebar store', () => {
  ...
  test('should patch changed groups and order without replacing unchanged groups', () => {
    ...
    useSidebarStore.getState().applyGroupsChangedMessage({
      ...
      groups: [
        createGroup('group-1', [createSession('session-1', 'groups'), createSession('session-4', 'scratch')]),
  ...
  test('should keep locally closed sessions hidden until the host snapshot drops them', () => {
    ...
    useSidebarStore
      .getState()
      .applySidebarMessage(
        createHydrateMessage([createGroup('group-1', [createSession('session-1', 'groups')])], { revision: 3 })
      );
  ...
  test('should keep local sleep state until the host snapshot confirms it', () => {
    useSidebarStore
      .getState()
      .applySidebarMessage(createHydrateMessage([createGroup('group-1', [createSession('session-1', 'groups')])]));
    ...
    useSidebarStore
      .getState()
      .applySidebarMessage(
        createHydrateMessage([createGroup('group-1', [createSession('session-1', 'groups')])], { revision: 2 })
      );
  ...
  test('should replace the session record when only snooze state changes', () => {
    ...
    expect(afterReschedule.sessionsById['session-1']?.snoozedUntil).toBe('2026-07-30T09:00:00.000Z');
  });
  ...
  */
  test('should normalize a partial hydrated settings object into the full schema shape', () => {
    ...
    // silently drop them.
    expect((settings as unknown as { gpuiTitlebarTipsReadIds?: string[] }).gpuiTitlebarTipsReadIds).toEqual([
