...
import type { SessionChatBlock, SessionChatMessage } from '../../shared/session-chat';
import {
...
function message(id: string, role: SessionChatMessage['role'], blocks: SessionChatBlock[]): SessionChatMessage {
...
describe('FIFO tool pairing (§6.6b)', () => {
  test('Nth result pairs with Nth call in document order', () => {
    const pairs = pairSessionChatToolBlocks([call('Read'), call('Bash'), result('read output'), result('bash output')]);
  ...
  test('limit caps pairs but keeps result ordinals aligned', () => {
    const pairs = pairSessionChatToolBlocks([call('A'), call('B'), result('ra'), result('rb')], 1);
    expect(pairs).toHaveLength(1);
  ...
  test('interleaved call/result/call/result pairs correctly', () => {
    const pairs = pairSessionChatToolBlocks([call('A'), result('ra'), call('B'), result('rb')]);
    expect(pairs[0]?.result?.output).toBe('ra');
...
describe('tool fold (§6.6b)', () => {
  test('consecutive tool-only messages fold into the preceding assistant turn', () => {
    const assistant = message('a1', 'assistant', [{ text: 'let me look', type: 'text' }, call('Read')]);
  ...
  test('tool-only messages fold into the preceding reasoning summary', () => {
    const reasoning = message('r1', 'reasoning', [{ text: 'Inspecting the renderer', type: 'text' }]);
    ...
    expect(folded[0]?.blocks.map((block) => block.type)).toEqual(['text', 'tool-call', 'tool-result']);
  });
  ...
  test('isToolOnly requires nonempty all-tool blocks', () => {
    ...
    expect(isToolOnlySessionChatMessage(message('m', 'assistant', [{ text: 't', type: 'text' }, call('A')]))).toBe(
      false
    );
  });
