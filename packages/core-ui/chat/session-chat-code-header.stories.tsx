...
const fence = (info: string, lines: readonly string[]): string => ['```' + info, ...lines, '```'].join('\n');
...
const FILENAME_ATTRIBUTE_FENCE = fence('md filename=README.md', ['# Ghostex', '', '- `gx f` searches prompt history']);
...
const NARROW_MARKDOWN = [
  ...
  '',
  fence('tsx title="apps/web/src/chat/session-chat-queued-prompts-button.tsx:118"', [
...
const WRAP_MARKDOWN = ['One command, one base64 blob, both far wider than the pane.', '', LONG_LINE_FENCE].join('\n');
...
function Pane({
  ...
  return (
    ...
    >
      ...
      <StaticList before={before} messages={messages} theme={theme} withEditorSurface={withEditorSurface} />
    </div>
  );
}
...
function WrapComparison({ theme }: { theme: 'dark' | 'light' }) {
  ...
  return (
    <div className='flex min-h-0 flex-1 gap-2'>
      {mounted > 0 ? <Pane label='wrap off — scrolls sideways' messages={WRAP_MESSAGES} theme={theme} /> : null}
      {mounted > 1 ? <Pane label='wrap on — every character on screen' messages={WRAP_MESSAGES} theme={theme} /> : null}
    </div>
  );
}
  ...
  return (
    <>
      ...
      {/* The min-height is inline for the same reason the narrow pane's width
          ...
          short window. */}
      <div className='flex h-screen flex-col gap-2 bg-[#0a0a0a] p-2' style={{ minHeight: '66rem' }}>
        <div className='flex min-h-0 flex-[3] gap-2'>
          <Pane label='after — filename + wrap, open, copy' messages={AFTER_MESSAGES} theme={theme} />
          <Pane before label='before — language + copy only' messages={BEFORE_MESSAGES} theme={theme} />
        </div>
        <div className='flex min-h-0 flex-[2] gap-2'>
          <WrapComparison theme={theme} />
        </div>
        <div className='flex min-h-0 flex-[3] gap-2'>
          <Pane label='385px — three actions, host with an editor' messages={NARROW_MESSAGES} narrow theme={theme} />
          <Pane
