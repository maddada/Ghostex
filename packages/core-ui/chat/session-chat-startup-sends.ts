...
import { normalizeSessionChatPendingText, type SessionChatPendingSend } from './session-chat-pending';
...
export function sessionChatPendingWithStartupSends(
  ...
  const entries = pending.filter(
    (entry) => entry.startupDelivery?.state !== 'failed' || queueIds.has(entry.startupDelivery.promptId)
  );
  ...
  for (const prompt of queue) {
    ...
    const entry =
      ...
      entries.find(
        (entry) =>
          !entry.queuedPromptId &&
          !entry.imagePaths?.length &&
          !matched.has(entry.id) &&
    ...
    if (entry) {
      ...
      if (
        entry.queuedPromptId !== prompt.id ||
        entry.startupDelivery?.state !== delivery.state ||
...
export function sessionChatFullQueueOrder(
  queue: readonly SessionChatQueuedPrompt[],
