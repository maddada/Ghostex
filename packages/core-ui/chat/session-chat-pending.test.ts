...
describe('pending prune (§10.3)', () => {
  ...
  test('two identical sends need two advanced occurrences', () => {
    ...
    const second = assignSessionChatPendingOccurrence([first], pendingEntry('p2', 'again'));
    ...
    const twoOccurrences = [...oneOccurrence, userMsg('u2', 'again', 1700), assistantMsg('a2', 1800)];
...
describe('content keys', () => {
  test('image-only sends key on the path list; whitespace collapses', () => {
    ...
    expect(sessionChatPendingContentKey({ imagePaths: ['/tmp/x.png'], text: '  ' })).toBe('images:["/tmp/x.png"]');
    expect(sessionChatPendingContentKey({ text: '' })).toBe('empty');
  });

  test('[Image #N] markers are stripped before matching', () => {
    expect(sessionChatPendingContentKey({ text: '[Image #1] describe this' })).toBe('text:describe this');
  });
});
