...
const STORY_MESSAGES: SessionChatMessage[] = [
  ...
  userTurn('user-3', 'And what happens to HTML that is not a disclosure? Assume the transcript is hostile.', 5_000),
...
const BEFORE_MESSAGES: SessionChatMessage[] = STORY_MESSAGES.map((message) => ({
  ...
  blocks: message.blocks.map((block) =>
    block.type === 'text' ? { ...block, text: block.text.replace(/<(\/?)(details|summary)\b/gi, '<$1$2​') } : block
  ),
}));

function ChatPane({ messages, theme }: { messages: SessionChatMessage[]; theme: 'dark' | 'light' }) {
  return (
