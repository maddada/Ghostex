...
import { sessionChatIdCollides, sessionChatShadowedId } from './session-chat-assembler';
...
export function replaceSessionChatMergerList(merger: SessionChatMerger, list: readonly SessionChatMessage[]): void {
 ...
 */
export function removeSessionChatMergerIds(merger: SessionChatMerger, ids: readonly string[]): boolean {
  if (ids.length === 0) {
    return false;
  }
