  ...
  const poll = async () => {
    ...
    try {
      const results = await Promise.allSettled(
        getAccountsConnections().map(async (connection) => ({
          connection,
          data: await connection.request({ operation: 'setupStatus', owner: accountSetupOwner() }),
        }))
      );
      ...
      /* A disconnected computer will be checked again on the next connection poll. */
    } finally {
      pending = false;
    }
  };
