...
const commandIconPickerSource = readFileSync(
  new URL("./command-icon-picker.tsx", import.meta.url),
  "utf8",
);
...
describe("command icon picker source", () => {
  test("closes the popover before parent editor state updates", () => {
     ...
     * focus.
     */
    const optionSelect = sourceBetween(
      commandIconPickerSource,
      "onSelect={() => {",
      "value={option.label}",
    );
    ...
    expect(optionSelect.indexOf("setIsOpen(false);")).toBeLessThan(
      optionSelect.indexOf("onIconChange(option.icon);"),
    );
  });
});
