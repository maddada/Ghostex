...
describe('filterPreviousSessions', () => {
  test('should fuzzy match aliases and secondary session text', () => {
    ...
    expect(filterPreviousSessions(previousSessions, 'ad pvs')).toMatchObject([{ historyId: 'history-1' }]);
    expect(filterPreviousSessions(previousSessions, 'cld')).toMatchObject([{ historyId: 'history-2' }]);
  });
  ...
  test('should optionally restrict results to selected session tags before searching', () => {
    ...
    expect(filterPreviousSessions(previousSessions, '', { sessionTags: ['untagged', 'todo'] })).toMatchObject([
      { historyId: 'history-2' },
      { historyId: 'history-3' },
    ]);
  });
...
describe('filterPreviousSessionsModalItems', () => {
  test('should hide browser page history from the previous sessions modal', () => {
    ...
    expect(filterPreviousSessionsModalItems(previousSessions)).toMatchObject([{ historyId: 'history-agent' }]);
  });
});
...
function createPreviousSession(overrides: Partial<SidebarPreviousSessionItem>): SidebarPreviousSessionItem {
  return {
    activity: 'idle',
    alias: 'Atlas',
    closedAt: '2026-03-24T10:00:00.000Z',
