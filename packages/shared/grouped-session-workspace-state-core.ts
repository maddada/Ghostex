...
export function normalizeGroupedSessionWorkspaceSnapshot(
  ...
  return {
    ...
    nextSessionNumber:
      typeof snapshot.nextSessionNumber === 'number' && snapshot.nextSessionNumber > 0 ? snapshot.nextSessionNumber : 1,
  };
}

export function getActiveGroup(snapshot: GroupedSessionWorkspaceSnapshot): SessionGroupRecord | undefined {
...
export function getGroupForSession(
  ...
  return snapshot.groups.find((group) => group.snapshot.sessions.some((session) => session.sessionId === sessionId));
}
