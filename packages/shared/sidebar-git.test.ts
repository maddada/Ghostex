...
describe('buildSidebarGitMenuItems', () => {
  ...
  test('should show sync with main only for worktree projects', () => {
    ...
    expect(buildSidebarGitMenuItems(baseState).map((item) => item.label)).not.toContain('Sync with Main');
    expect(buildSidebarGitMenuItems({ ...baseState, isWorktree: true }).map((item) => item.label)).toEqual([
      'Commit',
      'Push',
      'Create PR',
      'Sync with Main',
