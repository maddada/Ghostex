...
describe('getProjectSidebarCommandsState', () => {
  test('should return defaults when a project has no stored actions', () => {
    expect(getProjectSidebarCommandsState({}, 'missing-project')).toEqual(createDefaultProjectSidebarCommandsState());
  });
  ...
  test('should return the stored slice for a known project', () => {
    ...
    expect(getProjectSidebarCommandsState({ 'project-b': projectState }, 'project-b')).toEqual(projectState);
  });
});
