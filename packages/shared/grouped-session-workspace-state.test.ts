...
describe('normalizeGroupedSessionWorkspaceSnapshot', () => {
  ...
  test('should preserve existing two-digit session display ids', () => {
    ...
    expect(snapshot.groups[0]?.snapshot.sessions.map((session) => session.displayId)).toEqual(['00', '01']);
  });
});
...
describe('createSessionInWorkspace', () => {
  test('should create sessions in the active group and increment the workspace session counter', () => {
    ...
    expect(second.snapshot.groups[0]?.snapshot.sessions.map((session) => session.sessionId)).toEqual([
      'session-1',
      'session-2',
    ]);
...
describe('createGroupFromSessionInWorkspace', () => {
  test('should move the dragged session into a newly created active group', () => {
    ...
    expect(result.snapshot.groups[0]?.snapshot.sessions.map((session) => session.sessionId)).toEqual(['session-1']);
    expect(result.snapshot.groups[1]?.title).toBe('Group 2');
    expect(result.snapshot.groups[1]?.snapshot.sessions.map((session) => session.sessionId)).toEqual(['session-2']);
  });
});
...
describe('active-group preferences', () => {
  ...
  test('should rename the main group', () => {
    const result = renameGroupInWorkspace(createDefaultGroupedSessionWorkspaceSnapshot(), 'group-1', 'Client A');
  ...
  test('should reorder groups when the incoming order matches the current set', () => {
    ...
    expect(result.changed).toBe(true);
    expect(result.snapshot.groups.map((group) => group.groupId)).toEqual(['group-3', DEFAULT_MAIN_GROUP_ID, 'group-2']);
...
function createWorkspaceSnapshot(input: TestWorkspaceSnapshotInput): GroupedSessionWorkspaceSnapshot {
  return {
    ...createDefaultGroupedSessionWorkspaceSnapshot(),
    ...input,
