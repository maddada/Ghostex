...
describe("moveSidebarV2GroupRows", () => {
  it("moves a row before its target", () => {
    const next = moveSidebarV2GroupRows(
      [row("a"), row("b"), row("c")],
      "c",
      { groupId: "a", position: "before" },
    );
  ...
  it("treats an adjacent boundary as a no-op", () => {
    expect(
      moveSidebarV2GroupRows([row("a"), row("b")], "a", { groupId: "b", position: "before" }),
    ).toBeUndefined();
    expect(
      moveSidebarV2GroupRows([row("a"), row("b")], "b", { groupId: "a", position: "after" }),
    ).toBeUndefined();
  });

  it("ignores a drop onto the dragged row itself and unknown ids", () => {
    expect(
      moveSidebarV2GroupRows([row("a"), row("b")], "a", { groupId: "a", position: "after" }),
    ).toBeUndefined();
    expect(
      moveSidebarV2GroupRows([row("a"), row("b")], "zzz", { groupId: "b", position: "after" }),
    ).toBeUndefined();
    expect(
      moveSidebarV2GroupRows([row("a"), row("b")], "a", { groupId: "zzz", position: "after" }),
    ).toBeUndefined();
  });
});
...
describe("projectSidebarV2GroupOrderByMachine", () => {
  ...
  it("moves a project and its same-machine worktrees together, in machine order", () => {
    const projected = projectSidebarV2GroupOrderByMachine({
      ...
      rows: [
        row("other"),
        row("repo", ["repo", "repo-worktree-2", "repo-worktree-1"]),
        row("tail"),
      ],
      sourceGroupId: "repo",
