...
import {
  ...
  toSidebarV2Session,
  toSidebarV2SessionsFromGroups,
} from "./sidebar-v2-session";
...
describe("toSidebarV2Session", () => {
  ...
  test("derives the lifecycle state from the contract's own resolver", () => {
    expect(
      toSidebarV2Session(sidebarSession({ isRunning: false, isSleeping: true })).lifecycleState,
    ).toBe("sleeping");
    expect(
      toSidebarV2Session(sidebarSession({ isLive: false, isRunning: false })).lifecycleState,
    ).toBe("done");
  ...
  test("legacy browser rows keep their session kind", () => {
    ...
    expect(isSidebarV2BrowserSession(toSidebarV2Session(sidebarSession({ kind: "browser" })))).toBe(
      true,
    );
  ...
  test("a widened source row is read directly, so no adapter change is needed once the contract carries the fields", () => {
    expect(
      toSidebarV2Session({ ...sidebarSession(), createdAt: "2026-07-29T09:00:00.000Z" }).createdAt,
...
describe("toSidebarV2SessionsFromGroups", () => {
  ...
  test("flattens groups and stamps the owning project id", () => {
    expect(
      toSidebarV2SessionsFromGroups(groups).map((session) => [session.sessionId, session.projectId]),
    ).toEqual([
      ["a1", "project-a"],
      ["a2", "project-a"],
      ["b1", "project-b"],
