...
describe("sessionLastActivityAtMs", () => {
  ...
  test("falls through malformed stamps and reports null when nothing is usable", () => {
    expect(sessionLastActivityAtMs({ lastInteractionAt: "nope", workingStartedAt: iso(-1_000) })).toBe(
      NOW_MS - 1_000,
    );
...
describe("effectiveSidebarV2Settled", () => {
  test("attention and working hold a session active even when explicitly settled", () => {
    expect(
      effectiveSidebarV2Settled(
        session({ activity: "attention", settledOverride: "settled" }),
        settleOptions,
      ),
    ).toBe(false);
    expect(
      effectiveSidebarV2Settled(
        session({ activity: "working", settledOverride: "settled" }),
        settleOptions,
      ),
    ).toBe(false);
  });
  ...
  test("a running lifecycle never blocks a settle — that is pane liveness", () => {
    expect(
      effectiveSidebarV2Settled(
        session({ lifecycleState: "running", settledOverride: "settled" }),
        settleOptions,
      ),
    ).toBe(true);
  });
  ...
  test("the explicit override wins in both directions", () => {
    expect(effectiveSidebarV2Settled(session({ settledOverride: "settled" }), settleOptions)).toBe(
      true,
    );
    expect(
      effectiveSidebarV2Settled(
  ...
  test("inactivity past the window auto-settles, inside it does not", () => {
    expect(
      effectiveSidebarV2Settled(session({ lastInteractionAt: iso(-3 * DAY_MS - 1_000) }), settleOptions),
    ).toBe(true);
    expect(
      effectiveSidebarV2Settled(session({ lastInteractionAt: iso(-3 * DAY_MS + 1_000) }), settleOptions),
    ).toBe(false);
...
describe("resolveSidebarV2AutoSettleDecision", () => {
  test("reports why a session would auto-settle", () => {
    expect(
      resolveSidebarV2AutoSettleDecision(session({ lastInteractionAt: iso(-4 * DAY_MS) }), settleOptions),
    ).toEqual({ reason: "inactivity", shouldAutoSettle: true });
    expect(
...
describe("effectiveSidebarV2Snoozed", () => {
  ...
  test("missing or malformed data never hides a session", () => {
    ...
    expect(effectiveSidebarV2Snoozed(session({ snoozedUntil: "nope" }), { nowMs: NOW_MS })).toBe(
      false,
    );
  });
...
describe("resolveSidebarV2NextWakeAtMs", () => {
  ...
  test("no timer for a row that is already visible", () => {
    expect(
      resolveSidebarV2NextWakeAtMs(session({ snoozedUntil: iso(-1_000) }), { nowMs: NOW_MS }),
    ).toBeNull();
  });
});
