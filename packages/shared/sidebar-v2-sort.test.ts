...
describe('sortSessionsForSidebarV2', () => {
  ...
  test('equal creation stamps tie-break by session id, not by input order', () => {
    const sessions = [session('zulu', { createdAt: iso(0) }), session('alpha', { createdAt: iso(0) })];
...
describe('reconcileSidebarV2CreationOrder', () => {
  ...
  test('removed sessions drop out without disturbing the survivors', () => {
    expect(reconcileSidebarV2CreationOrder({ knownOrder: ['c', 'b', 'a'], sessionIds: ['a', 'c'] })).toEqual([
      'c',
      'a',
    ]);
  });
  ...
  test('several new sessions keep their incoming relative order at the top', () => {
    expect(reconcileSidebarV2CreationOrder({ knownOrder: ['a'], sessionIds: ['a', 'x', 'y'] })).toEqual([
      'x',
      'y',
      'a',
    ]);
  ...
  test('a reordered server payload cannot move an existing row', () => {
    expect(reconcileSidebarV2CreationOrder({ knownOrder: ['c', 'b', 'a'], sessionIds: ['a', 'b', 'c'] })).toEqual([
      'c',
      'b',
      'a',
    ]);
...
describe('settled and snoozed shelf sorts', () => {
  ...
  test('settled timestamp prefers settledAt, then the activity clock', () => {
    expect(
      resolveSidebarV2SettledTimestampMs(session('s', { lastInteractionAt: iso(-DAY_MS), settledAt: iso(-1_000) }))
    ).toBe(NOW_MS - 1_000);
    expect(
      resolveSidebarV2SettledTimestampMs(session('s', { lastInteractionAt: iso(-DAY_MS), settledAt: 'nope' }))
    ).toBe(NOW_MS - DAY_MS);
...
describe('partitionSidebarV2Sessions', () => {
  ...
  test('an older gxserver keeps everything in the inbox', () => {
    const result = partitionSidebarV2Sessions(
      [session('stale', { lastInteractionAt: iso(-90 * DAY_MS) }), session('snoozed', { snoozedUntil: iso(DAY_MS) })],
...
describe('resolveAdjacentSidebarV2SessionId', () => {
  ...
  test('walks the rendered order in both directions', () => {
    expect(resolveAdjacentSidebarV2SessionId({ currentSessionId: 'b', direction: 'next', sessionIds })).toBe('c');
    expect(resolveAdjacentSidebarV2SessionId({ currentSessionId: 'b', direction: 'previous', sessionIds })).toBe('a');
  });

  test('stops at the ends instead of wrapping', () => {
    expect(resolveAdjacentSidebarV2SessionId({ currentSessionId: 'c', direction: 'next', sessionIds })).toBeNull();
    expect(resolveAdjacentSidebarV2SessionId({ currentSessionId: 'a', direction: 'previous', sessionIds })).toBeNull();
  });

  test('no selection enters from the matching end', () => {
    expect(resolveAdjacentSidebarV2SessionId({ currentSessionId: null, direction: 'next', sessionIds })).toBe('a');
    expect(resolveAdjacentSidebarV2SessionId({ currentSessionId: null, direction: 'previous', sessionIds })).toBe('c');
  });

  test('an unknown current id yields null instead of guessing a neighbor', () => {
    expect(resolveAdjacentSidebarV2SessionId({ currentSessionId: 'gone', direction: 'next', sessionIds })).toBeNull();
    expect(resolveAdjacentSidebarV2SessionId({ currentSessionId: 'a', direction: 'next', sessionIds: [] })).toBeNull();
  });
});
...
describe('orderItemsByPreferredIds', () => {
  ...
  test('floats preferred ids to the front in the requested order', () => {
    expect(orderItemsByPreferredIds({ getId, items, preferredIds: ['c', 'a'] }).map(getId)).toEqual(['c', 'a', 'b']);
  });
  ...
  test('unknown and duplicate preferred ids are ignored', () => {
    expect(orderItemsByPreferredIds({ getId, items, preferredIds: ['zz', 'b', 'b'] }).map(getId)).toEqual([
      'b',
      'a',
      'c',
    ]);
