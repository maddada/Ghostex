    ...
    ...(input.path === undefined
      ...
      : {
          projectContext: {
            ...
            ...(input.gitRemoteOriginUrl === undefined
              ? {}
              : { gitRemoteOriginUrl: input.gitRemoteOriginUrl }),
...
describe("normalizeGitRemoteUrl", () => {
  ...
  test("keeps nested group paths, as GitLab needs", () => {
    expect(normalizeGitRemoteUrl("https://gitlab.com/group/sub/app.git")).toBe(
      "gitlab.com/group/sub/app",
    );
    expect(normalizeGitRemoteUrl("git@gitlab.com:group/sub/app.git")).toBe(
      "gitlab.com/group/sub/app",
    );
  });
...
describe("deriveSidebarV2RepositoryIdentity", () => {
  ...
  test("detects the other hosted providers", () => {
    expect(
      deriveSidebarV2RepositoryIdentity({ remoteUrl: "git@gitlab.com:a/b.git" })?.provider,
    ).toBe("gitlab");
    expect(
      deriveSidebarV2RepositoryIdentity({ remoteUrl: "git@bitbucket.org:a/b.git" })?.provider,
    ).toBe("bitbucket");
    expect(
...
describe("normalizeProjectPathForComparison", () => {
  test("trims trailing separators without touching POSIX case", () => {
    expect(normalizeProjectPathForComparison("/Users/madda/dev/Ghostex/")).toBe(
      "/Users/madda/dev/Ghostex",
    );
    expect(normalizeProjectPathForComparison("  /a/b  ")).toBe("/a/b");
    expect(normalizeProjectPathForComparison("/A/b")).not.toBe(
      normalizeProjectPathForComparison("/a/b"),
    );
  });

  test("folds Windows separators and case", () => {
    expect(normalizeProjectPathForComparison("C:/Users/Dev/App/")).toBe("c:\\users\\dev\\app");
    expect(normalizeProjectPathForComparison("\\\\server\\Share\\App")).toBe(
      "\\\\server\\share\\app",
    );
  });
...
describe("physical and override keys", () => {
  test("machine plus normalized path identifies one checkout", () => {
    expect(
      deriveSidebarV2PhysicalProjectKey(project({ machineId: "mac-1", path: "/dev/app/" })),
    ).toBe("mac-1:/dev/app");
  });

  test("a project with no path falls back to its own id, never to another project", () => {
    expect(deriveSidebarV2PhysicalProjectKey(project({ projectId: "quick" }))).toBe(
      "local:#quick",
    );
  ...
  test("overrides key by physical checkout", () => {
    ...
    expect(deriveSidebarV2ProjectGroupingOverrideKey(target)).toBe(
      deriveSidebarV2PhysicalProjectKey(target),
    );
    expect(
    ...
    expect(
      resolveSidebarV2ProjectGroupingMode(target, DEFAULT_SIDEBAR_V2_PROJECT_GROUPING_SETTINGS),
    ).toBe("repository");
  });
});
...
describe("deriveSidebarV2LogicalProjectKey", () => {
  ...
  test("'repository' merges every checkout of the repo across machines", () => {
    ...
    expect(deriveSidebarV2LogicalProjectKey(remoteCopy)).toBe(
      deriveSidebarV2LogicalProjectKey(main),
    );
    expect(deriveSidebarV2LogicalProjectKey(subPackage)).toBe(
      deriveSidebarV2LogicalProjectKey(main),
    );
  });
  ...
  test("'repositoryPath' keeps sub-paths apart but still merges across machines", () => {
    ...
    expect(deriveSidebarV2LogicalProjectKey(subPackage, options)).toBe(
      "github.com/ghostex/ghostex::packages/ui",
    );
    expect(deriveSidebarV2LogicalProjectKey(remoteCopy, options)).toBe(
  ...
  test("'separate' never merges", () => {
    ...
    expect(deriveSidebarV2LogicalProjectKey(main, options)).toBe(
      deriveSidebarV2PhysicalProjectKey(main),
    );
  ...
  test("non-git projects fall back to the physical key and never merge", () => {
    ...
    expect(deriveSidebarV2LogicalProjectKey(left)).not.toBe(
      deriveSidebarV2LogicalProjectKey(right),
    );
  });
...
describe("createSidebarV2ProjectGroupingSettings", () => {
  ...
  test("carries the user's overrides through untouched", () => {
    ...
    expect(
      createSidebarV2ProjectGroupingSettings(overrides).sidebarV2ProjectGroupingOverrides,
    ).toEqual(overrides);
  });
});
