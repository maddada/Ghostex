...
describe('orderProjectsWithWorktrees', () => {
  ...
  test('moving a main project carries its worktrees in the same relative order', () => {
    expect(
      orderProjectsWithWorktrees([worktree('wt-1'), worktree('wt-2'), project('other'), project('main')]).map(
        (item) => item.projectId
      )
...
describe('moveProjectsWithWorktrees', () => {
  test('allows worktree reordering inside the same main-project family', () => {
    expect(
      moveProjectsWithWorktrees([project('main'), worktree('wt-1'), worktree('wt-2'), project('other')], 'wt-2', {
        orderId: 'wt-1',
        position: 'before',
      }).map((item) => item.projectId)
  ...
  test('blocks worktree drops outside the main-project family', () => {
    ...
    expect(
      moveProjectsWithWorktrees(projects, 'wt-1', { orderId: 'other', position: 'after' }).map((item) => item.projectId)
