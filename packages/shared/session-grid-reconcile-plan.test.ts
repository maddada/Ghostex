...
describe('createVisibleSessionReconcilePlan', () => {
  ...
  test('should capture the minimal changed slots when a killed visible session is backfilled', () => {
    const currentSnapshot = createSnapshot([1, 2, 3, 4], ['session-1', 'session-2', 'session-3'], 'horizontal');
    const nextSnapshot = createSnapshot([1, 3, 4], ['session-1', 'session-3', 'session-4'], 'horizontal');
    const plan = createVisibleSessionReconcilePlan(currentSnapshot, nextSnapshot);
  ...
  test('should keep unaffected slots during sidebar reorder changes', () => {
    const currentSnapshot = createSnapshot([1, 2, 3, 4], ['session-1', 'session-2', 'session-3', 'session-4'], 'grid');
    const nextSnapshot = createSnapshot([1, 3, 2, 4], ['session-1', 'session-3', 'session-2', 'session-4'], 'grid');
    const plan = createVisibleSessionReconcilePlan(currentSnapshot, nextSnapshot);
  ...
  test('should rebuild when the requested layout shape changes', () => {
    ...
    const nextSnapshot = createSnapshot([1, 2, 3, 4], ['session-1', 'session-2', 'session-3', 'session-4'], 'grid');
    const plan = createVisibleSessionReconcilePlan(currentSnapshot, nextSnapshot);
  ...
  test('should rebuild when the view mode changes even if the visible count stays the same', () => {
    const currentSnapshot = createSnapshot([1, 2, 3], ['session-1', 'session-2', 'session-3'], 'horizontal');
...
function createSnapshot(
  ...
  return {
    ...
    sessions: orderedSessionNumbers.map((sessionNumber, slotIndex) => createSessionRecord(sessionNumber, slotIndex)),
    viewMode,
    visibleCount: toVisibleSessionCount(visibleSessionIds.length === 0 ? 1 : visibleSessionIds.length),
