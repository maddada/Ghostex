...
describe('resolveSidebarV2SnoozePresets', () => {
  test('morning offers all four presets', () => {
    ...
    expect(presets.map((preset) => preset.label)).toEqual(['In 1 hour', 'This evening', 'Tomorrow', 'Next week']);
  });

  test("'In 1 hour' is exactly one hour out", () => {
    const nowMs = localTime(2026, 7, 29, 9, 30);
    expect(byId(resolveSidebarV2SnoozePresets(nowMs), 'hour')?.snoozedUntilMs).toBe(nowMs + HOUR_MS);
  });
  ...
  test('the evening preset disappears once it is within an hour', () => {
    expect(byId(resolveSidebarV2SnoozePresets(localTime(2026, 7, 29, 17, 30)), 'evening')).toBeUndefined();
    expect(byId(resolveSidebarV2SnoozePresets(localTime(2026, 7, 29, 22, 0)), 'evening')).toBeUndefined();
    expect(byId(resolveSidebarV2SnoozePresets(localTime(2026, 7, 29, 16, 30)), 'evening')).toBeDefined();
  });

  test("'Tomorrow' is the next calendar day at the morning hour", () => {
    const tomorrow = byId(resolveSidebarV2SnoozePresets(localTime(2026, 7, 29, 22, 30)), 'tomorrow');
  ...
  test("'Tomorrow' rolls the month over", () => {
    const tomorrow = byId(resolveSidebarV2SnoozePresets(localTime(2026, 7, 31, 23, 30)), 'tomorrow');
    const tomorrowDate = new Date(tomorrow?.snoozedUntilMs ?? 0);
...
describe('formatSidebarV2SnoozeWakeDescription', () => {
  test('distinguishes today, tomorrow, this week, and later', () => {
    ...
    expect(formatSidebarV2SnoozeWakeDescription(localTime(2026, 7, 29, 17, 30), nowMs, options)).not.toMatch(
      /tomorrow|,/
    );
    expect(formatSidebarV2SnoozeWakeDescription(localTime(2026, 7, 30, 9, 0), nowMs, options)).toMatch(/^tomorrow /);
    expect(formatSidebarV2SnoozeWakeDescription(localTime(2026, 8, 3, 9, 0), nowMs, options)).toMatch(/^Mon /);
    expect(formatSidebarV2SnoozeWakeDescription(localTime(2026, 8, 20, 9, 0), nowMs, options)).toMatch(/, /);
  });
