...
function snapshot(overrides: Partial<GxserverPresentationSnapshot> = {}): GxserverPresentationSnapshot {
  return {
...
function project(overrides: Partial<GxserverPresentationProject> = {}): GxserverPresentationProject {
  return {
...
function session(sessionId: string, overrides: Partial<GxserverPresentationSession> = {}): GxserverPresentationSession {
  return {
    activity: 'idle',
    createdAt: '2026-06-02T07:18:00.000Z',
    groupId: 'Pmain:active',
...
function domainProject(overrides: Partial<GxserverProjectDomainState> = {}): GxserverProjectDomainState {
  return {
    attentionRules: {},
    completionRules: {},
    createdAt: '2026-06-02T07:18:00.000Z',
...
describe('gxserver presentation cache reducer', () => {
  ...
  test('updates the domain project cache only from project deltas', () => {
    const nextProjects = reduceGxserverProjectCacheForPresentationDelta([domainProject({ name: 'Old' })], {
  ...
  test('keeps unsnapshotted project sessions after saved manual order', () => {
    const next = reorderPresentationProjectSessions(
      snapshot({
        groups: [
          group({
            sessionIds: ['G1aaa' as GxserverSessionId, 'G2bbb' as GxserverSessionId, 'G3ccc' as GxserverSessionId],
          }),
        ],
        sessions: [
