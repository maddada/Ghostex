...
type BunSpawnSync = (options: { cmd: string[]; env?: Record<string, string>; stderr?: 'pipe'; stdout?: 'pipe' }) => {
  exitCode: number;
  stderr: Uint8Array;
  stdout: Uint8Array;
};
...
describe('cursor-cli-session', () => {
  ...
  test('should resolve the latest matching chat id for a project title', () => {
    ...
    try {
      ...
      const resolvedChatId = runNodeScript(getCursorChatSessionLookupScript(), [projectPath, title], {
        HOME: tempHome,
      }).trim();
